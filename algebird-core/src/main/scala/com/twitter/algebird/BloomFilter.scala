...
/** Bloom Filter - a probabilistic data structure to test presence of an
  * element.
  *
  * Operations 1) insert: hash the value k times, updating the bitfield at the
  * index equal to each hashed value 2) query: hash the value k times. If there
  * are k collisions, then return true; otherwise false.
  *
...
case class BloomFilterMonoid(numHashes: Int, width: Int, seed: Int)
  ...
  /** Assume the bloom filters are compatible (same width and same hashing
    * functions). This is the union of the 2 bloom filters.
    */
...
case class BFInstance(hashes: BFHash, bits: BitSet, width: Int) extends BF {
  ...
  /** Cardinality estimates are taken from Theorem 1 on page 15 of "Cardinality
    * estimation and dynamic length adaptation for Bloom filters" by Papapetrou,
    * Siberski, and Nejdl:
    * http://www.softnet.tuc.gr/~papapetrou/publications/Bloomfilters-DAPD.pdf
    *
    * Roughly, by using bounds on the expected number of true bits after n
    * elements have been inserted into the Bloom filter, we can go from the
    * actual number of true bits (which is known) to an estimate of the
    * cardinality.
    *
    * approximationWidth defines an interval around the maximum-likelihood
    * cardinality estimate. Namely, the approximation returned is of the form
    * (min, estimate, max) = ((1 - approxWidth) * estimate, estimate, (1 +
  ...
  /** s(n) is the expected number of bits that have been set to true after n
    * elements have been inserted into the Bloom filter. This is \hat{S}(n) in
    * the cardinality estimation paper used above.
    */
  ...
  /** sInverse(t) is the maximum likelihood value for the number of elements
    * that have been inserted into the Bloom filter when it has t bits set to
    * true. This is \hat{S}^{-1}(t) in the cardinality estimation paper used
    * above.
    */

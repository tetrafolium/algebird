...
/** A QTree provides an approximate Map[Double,A:Monoid] suitable for range
  * queries, quantile queries, and combinations of these (for example, if you
  * use a numeric A, you can derive the inter-quartile mean).
  *
  * It is loosely related to the Q-Digest data structure from
  * http://www.cs.virginia.edu/~son/cs851/papers/ucsb.sensys04.pdf, but using an
  * immutable tree structure, and carrying a generalized sum (of type A) at each
  * node instead of just a count.
  *
  * The basic idea is to keep a binary tree, where the root represents the
  * entire range of the input keys, and each child node represents either the
  * lower or upper half of its parent's range. Ranges are constrained to be
  * dyadic intervals
  * (https://en.wikipedia.org/wiki/Interval_(mathematics)#Dyadic_intervals) for
  * ease of merging.
  *
  * To keep the size bounded, the total count carried by any sub-tree must be at
  * least 1/(2^k) of the total count at the root. Any sub-trees that do not meet
  * this criteria have their children pruned and become leaves. (It's important
  * that they not be pruned away entirely, but that we keep a fringe of
  * low-count leaves that can gain weight over time and ultimately split again
  * when warranted).
  *
  * Quantile and range queries both give hard upper and lower bounds; the true
  * result will be somewhere in the range given.
  *
  * Keys must be >= 0.
  */
...
object QTree {
  ...
  /** level gives a bin size of 2^level. By default the bin size is 1/65536
    * (level = -16)
    */
  ...
  /** The common case of wanting an offset and sum for the same value This is
    * useful if you want to query the mean inside a range later. If you truly
    * just care about the counts/histogram, see the value method.
    */
  ...
  /** uses 1/65636 as the bin size, if you want to control that see other apply
    * or value methods.
    *
    * This is useful if you want to query the mean inside a range later. If you
    * truly just care about the counts/histogram, see the value method.
    */
  ...
  def unapply[A](
      qtree: QTree[A]
  ): Option[(Long, Int, Long, A, Option[QTree[A]], Option[QTree[A]])] =
    Some(
      (
        qtree.offset,
        qtree.level,
  ...
  /** If you are sure you only care about the approximate histogram features of
    * QTree, you can save some space by using QTree[Unit]
    */
  def value(v: Long): QTree[Unit] = apply(v -> ())

  /** If you are sure you only care about the approximate histogram features of
    * QTree, you can save some space by using QTree[Unit] level gives a bin size
    * of 2^level. By default this is 1/65536 (level = -16)
    */
  def value(v: Double, level: Int = DefaultLevel): QTree[Unit] =
    apply(v -> (), level)

  private[algebird] def mergePeers[@specialized(Int, Long, Float, Double) A](
      left: QTree[A],
      right: QTree[A]
  )(implicit monoid: Monoid[A]): QTree[A] = {
    assert(
      right.lowerBound == left.lowerBound,
      "lowerBound " + right.lowerBound + " != " + left.lowerBound
    )
    assert(
...
class QTreeSemigroup[A](k: Int)(implicit val underlyingMonoid: Monoid[A])
    extends Semigroup[QTree[A]] {

  /** Override this if you want to change how frequently sumOption calls
    * compress
    */
...
class QTree[@specialized(Int, Long, Float, Double) A] private[algebird] (
  ...
  override def toString: String =
    _root_.scala.runtime.ScalaRunTime._toString(this)
  ...
  @annotation.tailrec
  private[algebird] final def extendToLevel(
      n: Int
  )(implicit monoid: Monoid[A]): QTree[A] = {
    if (n <= level)
      this
  ...
  /** Find the smallest dyadic interval that contains the dyadic interval for
    * this tree's root and the other tree's root, and return its level (that is,
    * the power of 2 for the interval).
    */
  ...
  /** This merges with another QTree but DOES NOT compress. You should probably
    * never use this and instead use QTreeSemigroup.plus(a, b) or .sumOption.
    * Strongly prefer sumOption if you can, as it is much more efficient due to
    * compressing less frequently.
    */
  ...
  /** give lower and upper bounds respectively of the percentile value given.
    * For instance, quantileBounds(0.5) would give an estimate of the median.
    */
  ...
  /** Get the bounds on the sums within a range (not percentile) This along with
    * the rangeCountBounds can tell you the mean over a range
    */
  def rangeSumBounds(from: Double, to: Double)(implicit
      monoid: Monoid[A]
  ): (A, A) = {
  ...
  /** Users should never need to call this if they are adding QTrees using the
    * Semigroup This makes sure no element in the tree has count less than the
    * total count / 2^k. That means after this call there are at most 2^k nodes,
    * but usually fewer.
    */
  ...
  @inline
  private def pruneChild(minCount: Long, childNullable: QTree[A])(implicit
      m: Monoid[A]
  ): QTree[A] =
  ...
  /** How many total nodes are there in the QTree. Not meaningful for learning
    * statistics, but interesting to estimate serialization size.
    */
  def size: Int = {
  ...
  private def mapChildrenWithDefault[T](
      default: T
  )(fn: QTree[A] => T): (T, T) = {
    (
  ...
  /** This gives you the mean for the middle 50%-ile. This probably only makes
    * sense if the Monoid[A] is equivalent to addition in Numeric[A], which is
    * only used to convert to Double at the end
    */
  def interQuartileMean(implicit
      n: Numeric[A],
      m: Monoid[A]
  ): (Double, Double) = {
...
trait QTreeAggregatorLike[T] {
  ...
  /** We convert T to a Double, then the Double is converted to a Long by using
    * a 2^level bucket size.
    */
...
/** QTree aggregator is an aggregator that can be used to find the approximate
  * percentile bounds. The items that are iterated over to produce this
  * approximation cannot be negative. Returns an Intersection which represents
...
case class QTreeAggregator[T](
    ...
    extends Aggregator[
      T,
      QTree[Unit],
      Intersection[InclusiveLower, InclusiveUpper, Double]
    ]
...
/** QTreeAggregatorLowerBound is an aggregator that is used to find an
  * appoximate percentile. This is similar to a QTreeAggregator, but is a
  * convenience because instead of returning an Intersection, it instead returns
  * the lower bound of the percentile. Like a QTreeAggregator, the items that
  * are iterated over to produce this approximation cannot be negative.
  */

...
/** A Count-Min sketch is a probabilistic data structure used for summarizing
  ...
  * confidence in our error estimates, and let `d = ceil(ln 1/delta)` and
  * `w = ceil(e / eps)`.
  *
  ...
  *
  * With probability at least `1 - delta`, this estimate is within `eps * N` of
  * the true frequency (i.e.,
  * `true frequency <= estimate <= true frequency + eps * N`), where N is the
  * total size of the stream so far.
  *
  * See http://www.eecs.harvard.edu/~michaelm/CS222/countmin.pdf for technical
  * details, including proofs of the estimates and error bounds used in this
  * implementation.
  *
...
/** Monoid for adding [[CMS]] sketches.
  ...
  * =Usage=
  *
  * `eps` and `delta` are parameters that bound the error of each query
  * estimate. For example, errors in answering point queries (e.g., how often
  * has element x appeared in the stream described by the sketch?) are often of
  * the form: "with probability p >= 1 - delta, the estimate is close to the
  * truth by some factor depending on eps."
  *
  * The type `K` is the type of items you want to count. You must provide an
  * implicit `CMSHasher[K]` for `K`, and Algebird ships with several such
  * implicits for commonly used types such as [[Long]] and [[BigInt]]:
  *
  * {{{
  * import com.twitter.algebird.CMSHasherImplicits._
  * }}}
  *
  * If your type `K` is not supported out of the box, you have two options: 1)
  * You provide a "translation" function to convert items of your (unsupported)
  * type `K` to a supported type such as [[Double]], and then use the
  * `contramap` function of [[CMSHasher]] to create the required `CMSHasher[K]`
  * for your type (see the documentation of [[CMSHasher]] for an example); 2)
  * You implement a `CMSHasher[K]` from scratch, using the existing CMSHasher
  * implementations as a starting point.
  *
  * Note: Because Arrays in Scala/Java not have sane `equals` and `hashCode`
  * implementations, you cannot safely use types such as `Array[Byte]`. Extra
  * work is required for Arrays. For example, you may opt to convert `Array[T]`
  * to a `Seq[T]` via `toSeq`, or you can provide appropriate wrapper classes.
  * Algebird provides one such wrapper class, [[Bytes]], to safely wrap an
  * `Array[Byte]` for use with CMS.
  *
  * @param eps
  *   One-sided error bound on the error of each point query, i.e. frequency
  *   estimate.
  * @param delta
  *   A bound on the probability that a query estimate does not lie within some
  *   small interval (an interval that depends on `eps`) around the truth.
  * @param seed
  *   A seed to initialize the random number generator used to create the
  *   pairwise independent hash functions.
  * @tparam K
  *   The type used to identify the elements to be counted. For example, if you
  *   want to count the occurrence of user names, you could map each username to
  *   a unique numeric ID expressed as a `Long`, and then count the occurrences
  *   of those `Long`s with a CMS of type `K=Long`. Note that this mapping
  *   between the elements of your problem domain and their identifiers used for
  *   counting via CMS should be bijective. We require a [[CMSHasher]] context
  *   bound for `K`, see [[CMSHasherImplicits]] for available implicits that can
  *   be imported. Which type K should you pick in practice? For domains that
  *   have less than `2^64` unique elements, you'd typically use [[Long]]. For
  *   larger domains you can try [[BigInt]], for example. Other possibilities
  *   include Spire's `SafeLong` and `Numerical` data types
  *   (https://github.com/non/spire), though Algebird does not include the
  *   required implicits for CMS-hashing (cf. [[CMSHasherImplicits]].
  */
class CMSMonoid[K: CMSHasher](eps: Double, delta: Double, seed: Int)
  ...
  def plus(left: CMS[K], right: CMS[K]): CMS[K] = {
    require(
      left.params.hashes == right.params.hashes,
      "The sketches must use the same hash functions."
    )
    left ++ right
  }
  ...
  /** Creates a sketch out of multiple items.
    */
  def create(data: Seq[K]): CMS[K] = data.foldLeft(zero) { case (acc, x) =>
    plus(acc, create(x))
  }
...
/** An Aggregator for [[CMS]]. Can be created using [[CMS.aggregator]].
  */
case class CMSAggregator[K](cmsMonoid: CMSMonoid[K])
...
/** Configuration parameters for [[CMS]].
  *
  * @param hashes
  *   Pair-wise independent hashes functions. We need `N=depth` such functions
  *   (`depth` can be derived from `delta`).
  * @param eps
  *   One-sided error bound on the error of each point query, i.e. frequency
  *   estimate.
  * @param delta
...
object CMSFunctions {
  ...
  @throws[IllegalArgumentException](
    "if depth is too large, causing precision errors when computing delta"
  )
  def delta(depth: Int): Double = {
    val i = 1.0 / scala.math.exp(depth)
    require(
      i != 0.0,
      s"depth must be smaller as it causes precision errors when computing delta ($depth led to an invalid delta of 0.0)"
    )
    i
  }
  ...
  /** Generates `N=depth` pair-wise independent hash functions.
    ...
    *   estimate.
    * @param delta
    *   Error bound on the probability that a query estimate does NOT lie within
    *   some small interval around the truth.
    * @param seed
    ...
    */
  def generateHashes[K: CMSHasher](
      eps: Double,
      delta: Double,
      seed: Int
  ): Seq[CMSHash[K]] = {
...
/** A trait for CMS implementations that can count elements in a data stream and
  * that can answer point queries (i.e. frequency estimates) for these elements.
  *
...
trait CMSCounting[K, C[_]] {
  ...
  /** Returns the bound on the probability that a query estimate does NOT lie
    * within some small interval (an interval that depends on `eps`) around the
    * truth.
    */
  def delta: Double

  /** Number of hash functions (also: number of rows in the counting table).
    * This number is derived from `delta`.
    */
  def depth: Int = CMSFunctions.depth(delta)

  /** Number of counters per hash function (also: number of columns in the
    * counting table). This number is derived from `eps`.
    */
  def width: Int = CMSFunctions.width(eps)

  /** Returns a new sketch that is the combination of this sketch and the other
    * sketch.
    */
  def ++(other: C[K]): C[K]
  ...
  /** Returns an estimate of the total number of times this item has been seen
    ...
    *
    * It is always true that `estimatedFrequency >= trueFrequency`. With
    * probability `p >= 1 - delta`, it also holds that
  ...
  /** Returns an estimate of the inner product against another data stream.
    *
    * In other words, let a_i denote the number of times element i has been seen
    * in the data stream summarized by this CMS, and let b_i denote the same for
    * the other CMS. Then this returns an estimate of `<a, b> = \sum a_i b_i`.
    *
    * Note: This can also be viewed as the join size between two relations.
    *
    * It is always true that actualInnerProduct <= estimatedInnerProduct. With
    * probability `p >= 1 - delta`, it also holds that
  ...
  /** The second frequency moment is `\sum a_i^2`, where `a_i` is the count of
    * the i-th element.
    */
...
/** A trait for CMS implementations that can track heavy hitters in a data
  * stream.
  *
  * It is up to the implementation how the semantics of tracking heavy hitters
  * are defined. For instance, one implementation could track the "top %" heavy
  * hitters whereas another implementation could track the "top N" heavy
  * hitters.
  *
...
/** A Count-Min sketch data structure that allows for counting and frequency
  * estimation of elements in a data stream.
  *
  * Tip: If you also need to track heavy hitters ("Top N" problems), take a look
  * at [[TopCMS]].
  *
  * =Usage=
  *
  * This example demonstrates how to count [[Long]] elements with [[CMS]], i.e.
  * `K=Long`.
  *
  * Note that the actual counting is always performed with a [[Long]],
  * regardless of your choice of `K`. That is, the counting table behind the
  * scenes is backed by [[Long]] values (at least in the current
  * implementation), and thus the returned frequency estimates are always
  * instances of `Approximate[Long]`.
  *
  * @example
  *   {{{ // Implicits that enabling CMS-hashing of `Long` values. import
  *   com.twitter.algebird.CMSHasherImplicits._
  *
  * // Creates a monoid for a CMS that can count `Long` elements. val cmsMonoid:
  * CMSMonoid[Long] = { val eps = 0.001 val delta = 1E-10 val seed = 1
  * CMS.monoid[Long](eps, delta, seed) }
  *
  * // Creates a CMS instance that has counted the element `1L`. val cms:
  * CMS[Long] = cmsMonoid.create(1L)
  *
  * // Estimates the frequency of `1L` val estimate: Approximate[Long] =
  * cms.frequency(1L) }}}
  *
  * @tparam K
  *   The type used to identify the elements to be counted.
  */
sealed abstract class CMS[K](val params: CMSParams[K])
    extends java.io.Serializable
    with CMSCounting[K, CMS] {
...
/** Zero element. Used for initialization.
  */
case class CMSZero[K](override val params: CMSParams[K])
...
case class CMSItem[K](
  ...
  override def +(x: K, count: Long): CMS[K] =
    CMSInstance[K](params) + item + (x, count)
  ...
  override def frequency(x: K): Approximate[Long] =
    if (item == x) Approximate.exact(1L) else Approximate.exact(0L)

  override def innerProduct(other: CMS[K]): Approximate[Long] =
    other.frequency(item)

}

/** The general Count-Min sketch structure, used for holding any number of
  * elements.
  */
case class CMSInstance[K](
  ...
  def frequency(item: K): Approximate[Long] =
    makeApprox(
      countsTable.counts.iterator
        .zip(params.hashes.iterator)
        .map { case (row, hash) => row(hash(item)) }
        .min
    )

  /** Let X be a CMS, and let count_X[j, k] denote the value in X's
    * 2-dimensional count table at row j and column k. Then the Count-Min sketch
    * estimate of the inner product between A and B is the minimum inner product
    * between their rows: estimatedInnerProduct = min_j (\sum_k count_A[j, k] *
    * count_B[j, k]|)
    */
  def innerProduct(other: CMS[K]): Approximate[Long] = {
    other match {
      case other: CMSInstance[_] =>
        require(
          (other.depth, other.width) == (depth, width),
...
object CMSInstance {
  ...
  /** Initializes a [[CMSInstance]] with all zeroes, i.e. nothing has been
    * counted yet.
    */
  def apply[K](params: CMSParams[K]): CMSInstance[K] = {
    val countsTable = CountsTable[K](
      CMSFunctions.depth(params.delta),
      CMSFunctions.width(params.eps)
    )
    CMSInstance[K](countsTable, 0, params)
  }

  /** The 2-dimensional table of counters used in the Count-Min sketch. Each row
    * corresponds to a particular hash function.
    */
  ...
  case class CountsTable[K](counts: Vector[Vector[Long]]) {
    ...
    def getCount(pos: (Int, Int)): Long = {
      ...
      require(
        row < depth && col < width,
        "Position must be within the bounds of this table."
      )
      counts(row)(col)
    }
    ...
    def ++(other: CountsTable[K]): CountsTable[K] = {
      require(
        (depth, width) == (other.depth, other.width),
        "Tables must have the same dimensions."
      )
      val iil: IndexedSeq[IndexedSeq[Long]] =
        Monoid.plus[IndexedSeq[IndexedSeq[Long]]](counts, other.counts)
...
/** A Count-Min sketch data structure that allows for (a) counting and frequency
  * estimation of elements in a data stream and (b) tracking the heavy hitters
  * among these elements.
  *
  * The logic of how heavy hitters are computed is pluggable, see
  * [[HeavyHittersLogic]].
  *
  * Tip: If you do not need to track heavy hitters, take a look at [[CMS]],
  * which is more efficient in this case.
  *
  * =Usage=
  *
  * This example demonstrates how to count `Long` elements with [[TopCMS]], i.e.
  * `K=Long`.
  *
  * Note that the actual counting is always performed with a [[Long]],
  * regardless of your choice of `K`. That is, the counting table behind the
  ...
  *   com.twitter.algebird.CMSHasherImplicits._
  *
  * // Creates a monoid for a CMS that can count `Long` elements. val
  * topPctCMSMonoid: TopPctCMSMonoid[Long] = { val eps = 0.001 val delta = 1E-10
  * val seed = 1 val heavyHittersPct = 0.1 TopPctCMS.monoid[Long](eps, delta,
  * seed, heavyHittersPct) }
  *
  * // Creates a TopCMS instance that has counted the element `1L`. val topCMS:
  * TopCMS[Long] = topPctCMSMonoid.create(1L)
  *
  * // Estimates the frequency of `1L` val estimate: Approximate[Long] =
  * topCMS.frequency(1L)
  *
  * // What are the heavy hitters so far? val heavyHitters: Set[Long] =
  * topCMS.heavyHitters }}}
  *
  * @tparam K
  *   The type used to identify the elements to be counted.
...
/** Used for holding a single element, to avoid repeatedly adding elements from
  * sparse counts tables.
  */
case class TopCMSItem[K](
    item: K,
    override val cms: CMS[K],
    params: TopCMSParams[K]
) extends TopCMS[K](cms, params) {
...
class TopCMSMonoid[K](cms: CMS[K], logic: HeavyHittersLogic[K])
    extends Monoid[TopCMS[K]] {
...
abstract class HeavyHittersLogic[K] extends java.io.Serializable {
  ...
  def updateHeavyHitters(
      cms: CMS[K]
  )(left: HeavyHitters[K], right: HeavyHitters[K]) = {
...
/** Finds all heavy hitters, i.e., elements in the stream that appear at least
  ...
  *
  * Every item that appears at least `(heavyHittersPct * totalCount)` times is
  * output, and with probability `p >= 1 - delta`, no item whose count is less
  * than `(heavyHittersPct - eps) * totalCount` is output.
  *
  * This also means that this parameter is an upper bound on the number of heavy
  * hitters that will be tracked: the set of heavy hitters contains at most
  * `1 / heavyHittersPct` elements. For example, if `heavyHittersPct=0.01` (or
  * 0.25), then at most `1 / 0.01 = 100` items (or `1 / 0.25 = 4` items) will be
  * tracked/returned as heavy hitters. This parameter can thus control the
  * memory footprint required for tracking heavy hitters.
  */
case class TopPctLogic[K](heavyHittersPct: Double)
    extends HeavyHittersLogic[K] {

  require(
    0 < heavyHittersPct && heavyHittersPct < 1,
    "heavyHittersPct must lie in (0, 1)"
  )

  override def purgeHeavyHitters(
      cms: CMS[K]
  )(hitters: HeavyHitters[K]): HeavyHitters[K] = {
...
/** Tracks the top N heavy hitters, where `N` is defined by `heavyHittersN`.
  *
  * '''Warning:''' top-N computations are not associative. The effect is that a
  * top-N CMS has an ordering bias (with regard to heavy hitters) when merging
  * instances. This means merging heavy hitters across CMS instances may lead to
  * incorrect, biased results: the outcome is biased by the order in which CMS
  * instances / heavy hitters are being merged, with the rule of thumb being
  * that the earlier a set of heavy hitters is being merged, the more likely is
  * the end result biased towards these heavy hitters.
  *
  * @see
  *   Discussion in
  *   [[https://github.com/twitter/algebird/issues/353 Algebird issue 353]]
  */
...
/** Containers for holding heavy hitter items and their associated counts.
  */
case class HeavyHitters[K](hhs: Set[HeavyHitter[K]])
    extends java.io.Serializable {
...
/** Monoid for Top-% based [[TopCMS]] sketches.
  ...
  *
  * @param cms
  *   A [[CMS]] instance, which is used for the counting and the frequency
  *   estimation performed by this class.
  * @param heavyHittersPct
  *   A threshold for finding heavy hitters, i.e., elements that appear at least
  ...
  *   be imported. Which type K should you pick in practice? For domains that
  *   have less than `2^64` unique elements, you'd typically use [[Long]]. For
  *   larger domains you can try [[BigInt]], for example.
  */
...
object TopPctCMS {
  ...
  def monoid[K: CMSHasher](
    ...
    monoid(
      CMSFunctions.eps(width),
      CMSFunctions.delta(depth),
      seed,
      heavyHittersPct
    )
...
/** An Aggregator for [[TopPctCMS]]. Can be created using
  * [[TopPctCMS.aggregator]].
  */
case class TopPctCMSAggregator[K](cmsMonoid: TopPctCMSMonoid[K])
    extends TopCMSAggregator(cmsMonoid)

/** Monoid for top-N based [[TopCMS]] sketches. '''Use with care! (see warning
  * below)'''
  *
  * =Warning: Adding top-N CMS instances (`++`) is an unsafe operation=
  *
  * Top-N computations are not associative. The effect is that a top-N CMS has
  * an ordering bias (with regard to heavy hitters) when ''merging'' CMS
  * instances (e.g. via `++`). This means merging heavy hitters across CMS
  * instances may lead to incorrect, biased results: the outcome is biased by
  * the order in which CMS instances / heavy hitters are being merged, with the
  * rule of thumb being that the earlier a set of heavy hitters is being merged,
  * the more likely is the end result biased towards these heavy hitters.
  *
  * The warning above only applies when ''adding CMS instances'' (think:
  * `cms1 ++ cms2`). In comparison, heavy hitters are correctly computed when:
  *
  *   - a top-N CMS instance is created from a single data stream, i.e. `Seq[K]`
  *   - items are added/counted individually, i.e. `cms + item` or
  *     `cms + (item, count)`.
  *
  * See the discussion in
  * [[https://github.com/twitter/algebird/issues/353 Algebird issue 353]] for
  * further details.
  *
  * =Alternatives=
  *
  * The following, alternative data structures may be better picks than a top-N
  * based CMS given the warning above:
  *
  *   - [[TopPctCMS]]: Has safe merge semantics for its instances including
  *     heavy hitters.
  *   - [[SpaceSaver]]: Has the same ordering bias than a top-N CMS, but at
  *     least it provides bounds on the bias.
  *
  * =Usage=
  *
  * The type `K` is the type of items you want to count. You must provide an
...
/** K1 defines a scope for the CMS. For each k1, keep the top heavyHittersN
  ...
  */
case class ScopedTopNLogic[K1, K2](heavyHittersN: Int)
    extends HeavyHittersLogic[(K1, K2)] {
  ...
  override def purgeHeavyHitters(
    ...
    HeavyHitters[(K1, K2)](
      purged.values.flatten.toSet ++ underLimit.values.flatten.toSet
    )
  }
...
object ScopedTopNCMS {
  ...
  def monoid[K1: CMSHasher, K2: CMSHasher](
    ...
    new ScopedTopNCMSMonoid[K1, K2](
      CMS(eps, delta, seed)(scopedHasher[K1, K2]),
      heavyHittersN
    )
...
/** The Count-Min sketch uses `d` (aka `depth`) pair-wise independent hash
  ...
  * for the type `K` of the items you want to count. Algebird ships with several
  * such implicits for commonly used types `K` such as [[Long]] and [[BigInt]]:
  *
  * {{{
  * import com.twitter.algebird.CMSHasherImplicits._
  * }}}
  ...
  * type `K` to a supported type such as [[Double]], and then use the
  * `contramap` function of [[CMSHasher]] to create the required `CMSHasher[K]`
  * for your type (see the documentation of `contramap` for an example); 2) You
  * implement a `CMSHasher[K]` from scratch, using the existing CMSHasher
  * implementations as a starting point.
  */
trait CMSHasher[K] extends java.io.Serializable {
  ...
  /** Given `f`, a function from `L` into `K`, creates a `CMSHasher[L]` whose
    * hash function is equivalent to:
    *
    * {{{
  ...
  /** Given `f`, a function from `L` into `K`, creates a `CMSHasher[L]` whose
    ...
    * }}}
    *
    * Be aware that the use of contramap may come at a cost (e.g. increased
    * time) due to the translation calls between `K` and `L`.
    *
    * =Usage=
    *
...
/** Implicits that enable CMS-hashing for common data types such as [[Long]] and
  * [[BigInt]].
  */
object CMSHasherImplicits {
  ...
  /** =Implementation details=
    *
    * This hash function is based upon Murmur3. Note that the original CMS paper
    * requires `d` (depth) pair-wise independent hash functions; in the specific
    * case of Murmur3 we argue that it is sufficient to pass `d` different seed
    * values to Murmur3 to achieve a similar effect.
    *
    * To seed Murmur3 we use only `a`, which is a randomly drawn `Int` via
    * [[scala.util.Random]] in the CMS code. What is important to note is that
    * we intentionally ignore `b`. Why? We need to ensure that we seed Murmur3
    * with a random value, notably one that is uniformly distributed. Somewhat
    * surprisingly, combining two random values (such as `a` and `b` in our
    * case) typically worsens the "randomness" of the combination, i.e. the
    * combination is less uniformly distributed as either of its original
    * inputs. Hence the combination of two random values is discouraged in this
    * context, notably if the two random inputs were generated from the same
    * source anyways, which is the case for us because we use Scala's PRNG only.
    *
    * For further details please refer to the discussion
    * [[http://stackoverflow.com/questions/3956478/understanding-randomness Understanding Randomness]]
    * on StackOverflow.
    *
    * @param a
    *   Must be a random value, typically created via [[scala.util.Random]].
    * @param b
    *   Ignored by this particular hash function, see the reasoning above for
    *   the justification.
    * @param width
    *   Width of the CMS counting table, i.e. the width/size of each row in the
    *   counting table.
    * @param x
    *   Item to be hashed.
    * @return
    *   Slot assigned to item `x` in the vector of size `width`, where
    *   `x in [0, width)`.
    */

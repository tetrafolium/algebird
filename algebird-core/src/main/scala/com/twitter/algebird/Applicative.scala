...
object Applicative {
  ...
  def join[M[_], T, U](mt: M[T], mu: M[U])(
      implicit app: Applicative[M]
  ): M[(T, U)] =
    app.join(mt, mu)
  def join[M[_], T1, T2, T3](m1: M[T1], m2: M[T2], m3: M[T3])(
      implicit app: Applicative[M]
  ): M[(T1, T2, T3)] =
    app.join(m1, m2, m3)
  ...
  def sequence[M[_], T](
      ms: Seq[M[T]]
  )(implicit app: Applicative[M]): M[Seq[T]] =
    app.sequence(ms)
  def joinWith[M[_], T, U, V](mt: M[T], mu: M[U])(
      fn: (T, U) => V
  )(implicit app: Applicative[M]): M[V] =
...
/**
  ...
  */
class ApplicativeSemigroup[T, M[_]](

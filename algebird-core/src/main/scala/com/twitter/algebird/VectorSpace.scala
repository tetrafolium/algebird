...
object VectorSpace {
  ...
  def from[F, C[_]](
      scaleFn: (F, C[F]) => C[F]
  )(implicit fField: Field[F], cGroup: Group[C[F]]) = new VectorSpace[F, C] {
    def field = fField
    def group = cGroup
    def scale(v: F, c: C[F]) =
      if (field.isNonZero(v)) scaleFn(v, c) else cGroup.zero
  }
...
@implicitNotFound(
  msg = "Cannot find VectorSpace type class for Container: ${C} and Field: ${F}"
)

...
object MonadLaws {
  ...
  def leftIdentity[M[_], T, U](
      eq: (M[U], M[U]) => Boolean = defaultEq[M[U]]
  )(implicit
      monad: Monad[M],
      arb: Arbitrary[T],
  ...
  def associative[M[_], T, U, V](eq: (M[V], M[V]) => Boolean)(implicit
      monad: Monad[M],
      arb: Arbitrary[M[T]],
      fn1: Arbitrary[(T) => M[U]],
      fn2: Arbitrary[U => M[V]]
  ...
  def monadLaws[M[_], T, U, R](
      eq: (M[R], M[R]) => Boolean = defaultEq[M[R]]
  )(implicit
      monad: Monad[M],
      arb: Arbitrary[M[T]],
      fn1: Arbitrary[(T) => M[U]],
  ...
  implicit def indexedSeqA[T](implicit
      arbl: Arbitrary[List[T]]
  ): Arbitrary[IndexedSeq[T]] =
    Arbitrary { arbl.arbitrary.map { _.toIndexedSeq } }

  implicit def vectorA[T](implicit
      arbl: Arbitrary[List[T]]
  ): Arbitrary[Vector[T]] =

...
/** This is a typeclass to represent things which increase. Note that it is
  * important that a value after being incremented is always larger than it was
  * before. Note that next returns Option because this class comes with the
  * notion of the "greatest" key, which is None. Ints, for example, will cycle
  * if next(java.lang.Integer.MAX_VALUE) is called, therefore we need a notion
  * of what happens when we hit the bounds at which our ordering is violating.
  * This is also useful for closed sets which have a fixed progression.
  */
trait Successible[T] {
  ...
  /** The law is: next(t) .map { n => partialOrdering.lteq(t, n) &&
    * (!partialOrdering.equiv(t, n)) } .getOrElse(true)
    *
...
object Successible {
  ...
  /** This makes it easy to construct from a function when T has an ordering,
    * which is common Note, your function must respect the ordering
    */
  def fromNextOrd[T](
      nextFn: T => Option[T]
  )(implicit ord: Ordering[T]): Successible[T] = new Successible[T] {
  ...
  /** The difference between this and the default ordering on Option[T] is that
    * it treats None as the max value, instead of the minimum value.
    */
  ...
  implicit def partialFromOrdering[T](implicit
      ord: Ordering[T]
  ): PartialOrdering[T] = ord
}

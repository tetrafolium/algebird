...
case class Approximate[N](
    min: N,
    estimate: N,
    max: N,
    probWithinBounds: Double
)(implicit val numeric: Numeric[N]) {

  require(numeric.lteq(min, estimate) && numeric.lteq(estimate, max))

  /** Is this value contained within the bounds? Contract is:
  ...
  /** This is so you can do: val x = Approximate(1.0, 1.1, 1.2, 0.99) and then x
    * ~ 1.05 returns true
    */
  ...
  /** This is not distributive, because: a*(b+c) has two probability
    * multiplications while (a*b + a*b) has three Some kind of general formula
    * solver could possibly make this distributive, but in the mean time, it's
    * only a group
    */
  def *(right: Approximate[N]): Approximate[N] =

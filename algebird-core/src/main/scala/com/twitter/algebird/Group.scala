...
/** Group: this is a monoid that also has subtraction (and negation): So, you
  * can do (a-b), or -a (which is equal to 0 - a).
  */
...
/** Some(5) - Some(3) == Some(2) Some(5) - Some(5) == None negate Some(5) ==
  * Some(-5) Note: Some(0) and None are equivalent under this Group
  */
class OptionGroup[T](implicit group: Group[T])
    extends OptionMonoid[T]
    with Group[Option[T]] {
...
/** Extends pair-wise sum Array monoid into a Group negate is defined as the
  * negation of each element of the array.
  */
class ArrayGroup[T: ClassTag](implicit grp: Group[T])
    extends ArrayMonoid[T]()
    with Group[Array[T]] {
...
object Group extends GeneratedGroupImplicits with ProductGroups {
  ...
  def equiv[T](implicit grp: Group[T]): Equiv[T] = Equiv.fromFunction[T] {
    (a, b) =>
  ...
  implicit def mapGroup[K, V](implicit group: Group[V]) =
    new MapGroup[K, V]()(group)
  implicit def scMapGroup[K, V](implicit group: Group[V]) =

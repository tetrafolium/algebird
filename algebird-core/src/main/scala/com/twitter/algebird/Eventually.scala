...
/** Classes that support algebraic structures with dynamic switching between two
  * representations, the original type O and the eventual type E. In the case of
  * Semigroup, we specify
  *   - Two Semigroups eventualSemigroup and originalSemigroup
  *   - A Semigroup homomorphism convert: O => E
  *   - A conditional mustConvert: O => Boolean Then we get a
  *     Semigroup[Either[E,O]], where: Left(x) + Left(y) = Left(x+y) Left(x) +
  *     Right(y) = Left(x+convert(y)) Right(x) + Left(y) = Left(convert(x)+y)
  *     Right(x) + Right(y) = Left(convert(x+y)) if mustConvert(x+y) Right(x+y)
  *     otherwise. EventuallyMonoid, EventuallyGroup, and EventuallyRing are
  *     defined analogously, with the contract that convert respect the
...
class EventuallySemigroup[E, O](convert: O => E)(mustConvert: O => Boolean)(
  ...
  override def sumOption(
      iter: TraversableOnce[Either[E, O]]
  ): Option[Either[E, O]] = {
    iter.foldLeft[Either[Buffer[E], Buffer[O]]](Right(Buffer[O]()))(
      (buffer, v) => {
        ...
        def toEventualBuffer(buffer: Buffer[O]): Buffer[E] = {
          ...
          Semigroup
            .sumOption(buffer)
            .foreach((sum) => newBuffer += convert(sum))
          newBuffer
        }
...
trait EventuallyAggregator[A, E, O, C]
  ...
  @transient lazy val semigroup =
    new EventuallySemigroup[E, O](convert)(mustConvert)(
      leftSemigroup,
      rightAggregator.semigroup
    )
}
...
trait EventuallyMonoidAggregator[A, E, O, C]
  ...
  @transient lazy val monoid =
    new EventuallyMonoid[E, O](convert)(mustConvert)(
      leftSemigroup,
      rightAggregator.monoid
    )
}

...
/** Monoid (take a deep breath, and relax about the weird name): This is a
  * semigroup that has an additive identity (called zero), such that a+0=a,
  * 0+a=a, for every a
  */
...
/** Pair-wise sum Array monoid.
  *
  * plus returns left[i] + right[i] for all array elements. The resulting array
  * will be as long as the longest array (with its elements duplicated) zero is
  * an empty array
  */
class ArrayMonoid[T: ClassTag](implicit semi: Semigroup[T])
...
/** Function1 monoid. plus means function composition, zero is the identity
  * function
  */
...
object Monoid extends GeneratedMonoidImplicits with ProductMonoids {
  ...
  /** Return an Equiv[T] that uses isNonZero to return equality for all zeros
    * useful for Maps/Vectors that have many equivalent in memory
    * representations of zero
    */
  def zeroEquiv[T: Equiv: Monoid]: Equiv[T] = Equiv.fromFunction {
    (a: T, b: T) =>
      (!isNonZero(a) && !isNonZero(b)) || Equiv[T].equiv(a, b)
  }

  /** Same as v + v + v .. + v (i times in total) requires i >= 0, wish we had
    * NonnegativeBigInt as a class
    */
  def intTimes[T](i: BigInt, v: T)(implicit mon: Monoid[T]): T = {
    require(
      i >= 0,
      "Cannot do negative products with a Monoid, try Group.intTimes"
    )
    if (i == 0) {
      mon.zero
    } else {

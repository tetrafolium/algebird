...
/** Monad to handle mutating input state and possible failures. This is used to
  * interact in the planning phase with existing mutable APIs (like storm or
  * cascading), but retain the ability to compose carefully.
  */
sealed trait StateWithError[S, +F, +T] {
  ...
  def flatMap[F1 >: F, U](
      next: T => StateWithError[S, F1, U]
  ): StateWithError[S, F1, U] =
...
final case class StateFn[S, F, T](fn: S => Either[F, (S, T)])
    extends StateWithError[S, F, T] {
  def run(state: S) = fn(state)
}

/** A Trampolining instance that should prevent stack overflow at the expense of
  * performance
  */
final case class FlatMappedState[S, F, T, U](
    start: StateWithError[S, F, T],
    fn: T => StateWithError[S, F, U]
) extends StateWithError[S, F, U] {
  def run(state: S): Either[F, (S, U)] = {
    @annotation.tailrec
    def loop(
        inState: S,
        st: StateWithError[S, F, Any],
...
object StateWithError {
  ...
  def getState[S]: StateWithError[S, Nothing, S] = StateFn({ (state: S) =>
    Right(state, state)
  })
  def putState[S](newState: S): StateWithError[S, Nothing, Unit] = StateFn({
    (_: S) => Right(newState, ())
  })
  def swapState[S](newState: S): StateWithError[S, Nothing, S] = StateFn({
    (old: S) => Right(newState, old)
  })

  def const[S, T](t: T): StateWithError[S, Nothing, T] = StateFn({ (state: S) =>
    Right(state, t)
  })
  def lazyVal[S, T](t: => T): StateWithError[S, Nothing, T] = StateFn({
    (state: S) => Right(state, t)
  })
  def failure[S, F](f: F): StateWithError[S, F, Nothing] = StateFn({
    (state: S) => Left(f)
  })

  /** Use like fromEither[Int](Right("good")) to get a constant Either in the
    * monad
    */
  ...
  implicit def apply[S, F, T](
      fn: S => Either[F, (S, T)]
  ): StateWithError[S, F, T] = StateFn(fn)
  implicit def monad[S, F]
      : Monad[({ type Result[T] = StateWithError[S, F, T] })#Result] =
    new StateFMonad[F, S]

  class StateFMonad[F, S]
      extends Monad[({ type Result[T] = StateWithError[S, F, T] })#Result] {

    def apply[T](const: T) = { (s: S) => Right((s, const)) }
    def flatMap[T, U](earlier: StateWithError[S, F, T])(
        next: T => StateWithError[S, F, U]

...
/** This is a summing cache whose goal is to grow until we run out of memory, at
  * which point it clears itself and stops growing. Note that we can lose the
  * values in this cache at any point; we don't put anything here we care about.
  */
...
/** This is a wrapper around SummingCache that attempts to grow the capacity by
  * up to some maximum, as long as there's enough RAM. It determines that
  * there's enough RAM to grow by maintaining a SentinelCache which keeps
  * caching and summing the evicted values. Once the SentinelCache has grown to
  * the same size as the current cache, plus some margin, without running out of
  * RAM, then this indicates that we have enough headroom to double the
  * capacity.
  */
class AdaptiveCache[K, V: Semigroup](
  ...
  override def put(m: Map[K, V]): Option[Map[K, V]] = update(
    summingCache.put(m)
  )

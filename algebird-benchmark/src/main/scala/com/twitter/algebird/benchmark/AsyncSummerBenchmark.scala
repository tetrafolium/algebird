...
object AsyncSummerBenchmark {
  ...
  def hll[T](
      t: T
  )(implicit monoid: HyperLogLogMonoid, inj: Injection[T, Array[Byte]]): HLL =
    monoid.create(inj(t))
  ...
  class SummerState {
    ...
    def setup(): Unit = {
      ...
      nullSummer =
        new NullSummer[Long, HLL](Counter("tuplesIn"), Counter("tuplesOut"))
      ...
      val inputData: IndexedSeq[(Long, HLL)] = (0L until numInputItems).map {
        inputKey =>
      ...
      inputItems =
        inputData.grouped(bufferSize.v / 4).toIndexedSeq // materialize this
...
class AsyncSummerBenchmark {
  ...
  def fn(
      state: SummerState,
      summer: AsyncSummer[(Long, HLL), Map[Long, HLL]]
  ) = {

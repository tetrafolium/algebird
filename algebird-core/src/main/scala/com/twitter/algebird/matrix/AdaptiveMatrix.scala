...
/** A Matrix structure that is designed to hide moving between sparse and dense
  * representations Initial support here is focused on a dense row count with a
  * sparse set of columns
  */
...
object AdaptiveMatrix {
  ...
  implicit def monoid[V: Monoid]: Monoid[AdaptiveMatrix[V]] =
    new Monoid[AdaptiveMatrix[V]] {
      ...
      override def plus(a: AdaptiveMatrix[V], b: AdaptiveMatrix[V]) = sumOption(
        List(a, b)
      ).get
      ...
      private def sparseUpdate(
          storage: IndexedSeq[MMap[Int, V]],
          other: SparseColumnMatrix[V]
      ) = {
        other.rowsByColumns.zipWithIndex.foreach { case (contents, indx) =>
          ...
          AdaptiveVector.toMap(contents).foreach { case (col, value) =>
            curMap.update(
              col,
              Monoid.plus(value, curMap.getOrElse(col, innerZero))
            )
          }
        }
      }
      ...
      override def sumOption(
          items: TraversableOnce[AdaptiveMatrix[V]]
      ): Option[AdaptiveMatrix[V]] =
        if (items.isEmpty) {
          None
        } else {

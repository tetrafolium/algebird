...
object Aggregator extends java.io.Serializable {
  ...
  def prepareMonoid[F, T](prep: F => T)(
      implicit m: Monoid[T]
  ...
  /**
    ...
    */
  def approximatePercentile[T](percentile: Double, k: Int)(
  ...
  /**
    ...
    * The items that are iterated over cannot be negative.
    */
  def approximatePercentileBounds[T](percentile: Double, k: Int)(
      implicit num: Numeric[T]
  ): QTreeAggregator[T] =
...
class AggregatorApplicative[I]
  ...
  override def map[T, U](
      mt: Aggregator[I, _, T]
  )(fn: T => U): Aggregator[I, _, U] =

...
/** Simple implementation of a Monad type-class. Subclasses only need to
  * override apply and flatMap, but they should override map, join, joinWith,
  * and sequence if there are better implementations.
  *
  * Laws Monads must follow: identities: flatMap(apply(x))(fn) == fn(x)
  * flatMap(m)(apply _) == m associativity on flatMap (you can either flatMap f
  * first, or f to g: flatMap(flatMap(m)(f))(g) == flatMap(m) { x =>
  * flatMap(f(x))(g) }
  */
...
object Monad {
  ...
  def map[M[_], T, U](m: M[T])(fn: (T) => U)(implicit monad: Monad[M]) =
    monad.map(m)(fn)
  def foldM[M[_], T, U](acc: T, xs: GenTraversable[U])(
      fn: (T, U) => M[T]
  )(implicit monad: Monad[M]): M[T] =
    if (xs.isEmpty)
      monad.apply(acc)
    else
...
/** This enrichment allows us to use our Monad instances in for expressions: if
  * (import Monad._) has been done
  */
class MonadOperators[A, M[_]](m: M[A])(implicit monad: Monad[M])
    extends ApplicativeOperators[A, M](m) {

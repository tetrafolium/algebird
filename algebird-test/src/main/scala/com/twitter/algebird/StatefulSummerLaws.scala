...
object StatefulSummerLaws {
  ...
  def zeroEquiv[V: Semigroup: Equiv](v0: Option[V], v1: Option[V]): Boolean = {
    ...
    if (!(v0NonZero || v1NonZero)) {
      ...
      (for (a <- v0; b <- v1; eq = Equiv[V].equiv(a, b))
        yield eq).getOrElse(false)
    }
  }
  def sumIsPreserved[V: Semigroup: Equiv](
      summer: StatefulSummer[V],
      items: Iterable[V]
  ): Boolean = {
    summer.flush
    val sg = Semigroup.sumOption(items)
    val wsummer = Monoid.plus(
      Monoid.sum(items.map { summer.put(_) }.filter { _.isDefined }),
      summer.flush
    )
    zeroEquiv(sg, wsummer) && summer.isFlushed
  }
  // Law 2:
  def isFlushedIsConsistent[V](
      summer: StatefulSummer[V],
      items: Iterable[V]
  ): Boolean = {
    items.forall { v =>

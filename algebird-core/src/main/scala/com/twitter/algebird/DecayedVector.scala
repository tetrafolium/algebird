...
/** Represents a container class together with time. Its monoid consists of
  * exponentially scaling the older value and summing with the newer one.
  */
object DecayedVector {

  def buildWithHalflife[C[_]](
      vector: C[Double],
      time: Double,
      halfLife: Double
  ) = {
  ...
  def monoidWithEpsilon[C[_]](
      eps: Double
  )(implicit vs: VectorSpace[Double, C], metric: Metric[C[Double]]) =
  ...
  def mapMonoidWithEpsilon[K](eps: Double)(implicit
  ...
  def scaledPlus[C[_]](
      newVal: DecayedVector[C],
      oldVal: DecayedVector[C],
      eps: Double
  )(implicit

...
class SGDLaws extends CheckProperties {
  ...
  val sgdPosGen =
    for (x <- Gen.choose(0.0, 1.0);
      n <- Gen.choose(0.0, 0.001)) yield SGDPos((m * x + b + n, IndexedSeq(x)))

  val sgdWGen =
    for (cnt <- Gen.choose(0L, 100000L);
      m <- Gen.choose(-10.0, 10.0);
      b <- Gen.choose(-10.0, 10.0)) yield SGDWeights(cnt, Vector(m, b))
  ...
  property("unit step can be undone by adding gradient") {
    forAll { (w: SGDWeights, pos: SGDPos[(Double, IndexedSeq[Double])]) =>
      ...
      next.weights == minus(
        w.weights,
        SGD.linearGradient(w.weights, pos.pos.head)
      )
    }
  }
}

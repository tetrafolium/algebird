...
object Max {
  ...
  implicit def listMonoid[T: Ordering]: Monoid[Max[List[T]]] =
...
object First {
  implicit def semigroup[T] = Semigroup.from[First[T]] { (l, r) =>
    l
  }
}
...
object Last {
  implicit def semigroup[T] = Semigroup.from[Last[T]] { (l, r) =>
    r
  }
}

case class MinAggregator[T](implicit ord: Ordering[T])
    extends Aggregator[T, T, T] {
  def prepare(v: T) = v
  val semigroup = Semigroup.from { (l: T, r: T) =>
    ord.min(l, r)
  }
  def present(v: T) = v
}

case class MaxAggregator[T](implicit ord: Ordering[T])
    extends Aggregator[T, T, T] {
  def prepare(v: T) = v
  val semigroup = Semigroup.from { (l: T, r: T) =>
    ord.max(l, r)

...
object AdaptiveVector {
        ...
        (left, right) match {
          ...
          case _ if valueIsNonZero(left.sparseValue) =>
            fromVector(
              Vector(
                Semigroup.plus(
                  toVector(left): IndexedSeq[V],
                  toVector(right): IndexedSeq[V]
                ): _*
              ),
              left.sparseValue
            )
          case _ => // sparse is zero:
            fromMap(
              Semigroup.plus(toMap(left), toMap(right)),
              left.sparseValue,
              maxSize
            )
        }
  ...
  private class AVMonoid[V: Monoid]
      extends AVSemigroup[V]
      with Monoid[AdaptiveVector[V]] {
  ...
  private class AVGroup[V: Group]
      extends AVMonoid[V]
      with Group[AdaptiveVector[V]] {
...
/** An IndexedSeq that automatically switches representation between dense and
  * sparse depending on sparsity Should be an efficient representation for all
  * sizes, and it should not be necessary to special case immutable algebras
  * based on the sparsity of the vectors.
  */
...
case class DenseVector[V](
    iseq: Vector[V],
    override val sparseValue: V,
  ...
  def extend(cnt: Int) = {
    ...
    if (denseCount < newSize * AdaptiveVector.THRESHOLD) {
      // Go sparse
      SparseVector(
        AdaptiveVector.toMap(iseq, sparseValue),
        sparseValue,
        newSize
      )
    } else {
      // Stay dense
      DenseVector(
        iseq ++ Vector.fill(cnt)(sparseValue),
        sparseValue,
        denseCount
      )
    }
  ...
  def updated(idx: Int, v: V): AdaptiveVector[V] = {
    if (v == sparseValue) {
      ...
      if (newM.size >= size * AdaptiveVector.THRESHOLD) {
        // Go dense:
        DenseVector(
          AdaptiveVector.toVector(newM, sparseValue, size),
          sparseValue,
          newM.size
        )
      } else {

...
/** This Monoid allows code to depends on the results of asynchronous
  * computation. It is relatively common to have code which takes a Monoid and
  * elements, but applies the computation in an opaque way (a cache, for
  * example). This allows the code handing over the elements (in this case,
  * Tunnel objects) to depend on the result of the Monoid's computation. Note
  * that this code does not depend on any particular Monoid -- that dependency
  * is strictly when the Tunnel objects are created. This is the async analogue
  * of Function1Monoid.
  */
...
/** The tunnel class represents a piece of computation that depends on the
  * fulfilment of a promise. IMPORTANT: see apply, but Tunnels are mutable, and
  * can only be fulfilled once. They are generally not reusable. Reusing a
  * Tunnel in computation by a TunnelMonoid will cause the promise to be
...
case class Tunnel[V](future: Future[V], promise: Promise[V]) {
  ...
  /** This takes in a value and updates the promise, fulfilling the chain of
    * futures which depends on this final promise. IMPORTANT: this can only be
    * called once. In this way, it is dangerous to reuse Tunnel objects in
    * Monoid code that might reuse objects.
    */
...
object Tunnel {
  ...
  /** This lifts a value into a Tunnel. This is where the Monoidic computation
    * underlying a TunnelMonoid actually happens.
    */
  ...
  /** This attempts to fulfil the promise. If it has already been fulfilled,
    * this will throw an error if the value is different from the previous value
    * that was used.
    */

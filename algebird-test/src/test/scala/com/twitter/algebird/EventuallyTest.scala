...
class EventuallyRingLaws extends CheckProperties {
  ...
  val eventuallyRing =
    new EventuallyRing[Long, Int](_.asInstanceOf[Long])(_ > 10000)
...
class EventuallyTest extends WordSpec with Matchers {
  ...
  "EventuallyMonoid" should {
    ...
    "add R to R" in {
      assert(
        eventuallyMonoid.plus(Right(short), Right(short)) == Right(
          short + short
        )
      )
      assert(
        eventuallyMonoid.plus(Right(med), Right(med)) == Left(2 * med.length)
      )
      assert(
        eventuallyMonoid.plus(Right(short), Right(long)) == Left(
    ...
    "add L to R" in {
      assert(
        eventuallyMonoid.plus(Left(short.length), Right(short)) == Left(
          2 * short.length
        )
      )
      assert(
        eventuallyMonoid.plus(Left(short.length), Right(long)) == Left(
          short.length + long.length
        )
      )
    }
    ...
    "add R to L" in {
      assert(
        eventuallyMonoid.plus(Right(short), Left(short.length)) == Left(
          2 * short.length
        )
      )
      assert(
        eventuallyMonoid.plus(Right(long), Left(short.length)) == Left(
          short.length + long.length
        )
      )
    }
    ...
    "add L to L" in {
      assert(
        eventuallyMonoid.plus(Left(short.length), Left(short.length)) == Left(
          2 * short.length
        )
      )
      assert(
        eventuallyMonoid.plus(Left(long.length), Left(short.length)) == Left(
          short.length + long.length
        )
      )
    }
    ...
    "sumOption L,L,L" in {
      assert(
        eventuallyMonoid.sum(
          List(Left(short.length), Left(short.length), Left(short.length))
        ) == Left(3 * short.length)
      )
      assert(
        eventuallyMonoid.sum(
          List(Left(long.length), Left(short.length), Left(short.length))
        ) == Left(2 * short.length + long.length)
      )
    }
    ...
    "sumOption L,R,L" in {
      assert(
        eventuallyMonoid.sum(
          List(Left(short.length), Right(short), Left(short.length))
        ) == Left(3 * short.length)
      )
      assert(
        eventuallyMonoid.sum(
          List(Left(long.length), Right(short), Left(short.length))
        ) == Left(2 * short.length + long.length)
      )
    }
    ...
    "sumOption R,R,R" in {
      assert(
        eventuallyMonoid.sum(
          List(Right(short), Right(short), Right(short))
        ) == Right(short + short + short)
      )
      assert(
        eventuallyMonoid.sum(
          List(Right(long), Right(short), Right(short))
        ) == Left(2 * short.length + long.length)
      )
    }
    ...
    "sumOption 1010 R, L ,R" in {
      assert(
        eventuallyMonoid.sum(
          listOfRights :+ Left(short.length) :+ Right(short)
        ) == Left(1012 * short.length)
      )
    }
    ...
    "sumOption 1010 L, R ,L" in {
      assert(
        eventuallyMonoid.sum(
          listOfLefts :+ Right(short) :+ Left(short.length)
        ) == Left(1012 * short.length)
      )
    }
...
class EventuallyAggregatorLaws
    extends PropSpec
    with PropertyChecks
    with Matchers {
  ...
  def eventuallyAggregator(
      rightAg: Aggregator[Int, Int, Int]
  )(pred: (Int => Boolean)): EventuallyAggregator[Int, Double, Int, String] = {
  ...
  def isConvertedCorrectly(
      s: String,
      semi: EventuallySemigroup[Double, Int],
      in: List[Right[Double, Int]]
  ) = {
  ...
  property("EventuallyAggregator converts correctly") {
    forAll {
      (
        ...
        assert(
          in.isEmpty || isConvertedCorrectly(eventuallyAg(in), semi, rightIn)
        )
    }
  }

...
/** Hashes an arbitrary key type to one that the Sketch Map can use.
  */
case class SketchMapHash[K](hasher: CMSHash[Long], seed: Int)(implicit
...
class SketchMapMonoid[K, V](val params: SketchMapParams[K])(implicit
  ...
  /** A zero Sketch Map is one with zero elements.
    */
  val zero: SketchMap[K, V] = SketchMap(
    AdaptiveMatrix.fill(params.depth, params.width)(monoid.zero),
    Nil,
    monoid.zero
  )
  ...
  override def sumOption(
      items: TraversableOnce[SketchMap[K, V]]
  ): Option[SketchMap[K, V]] =
    if (items.isEmpty) None
      ...
      if (buffer.size > 1)
        sumBuffer // don't bother to sum if there is only one item.
      Some(buffer(0))
    }
  ...
  def create(data: Seq[(K, V)]): SketchMap[K, V] = {
    ...
    SketchMap(
      newTable,
      params.updatedHeavyHitters(heavyHitters, newTable),
      totalValue
    )
  }
...
case class SketchMapParams[K](
  ...
  lazy val hashes: Seq[K => Int] = {
    ...
    (0 to (numHashes - 1)).map { _ =>
      val smhash: SketchMapHash[K] =
        SketchMapHash(CMSHash[Long](r.nextInt, 0, numCounters), seed)(
          serialization
        )
  ...
  /** Returns a new set of sorted and concatenated heavy hitters given an
    ...
    */
  def updatedHeavyHitters[V: Ordering](
      hitters: Seq[K],
      table: AdaptiveMatrix[V]
  ): List[K] = {
    val mapping: Map[K, V] =
...
object SketchMapParams {
  ...
  def apply[K](seed: Int, eps: Double, delta: Double, heavyHittersCount: Int)(
    ...
    SketchMapParams[K](seed, width(eps), depth(delta), heavyHittersCount)(
      serialization
    )

  /** Functions to translate between (eps, delta) and (depth, width). The
    * translation is: depth = ceil(ln 1/delta) width = ceil(e / eps)
    */
...
object SketchMap {
  ...
  /** Generates a monoid used to create SketchMap instances. Requires a
    ...
    * monoid for V.
    */
  def monoid[K, V](params: SketchMapParams[K])(implicit
      valueOrdering: Ordering[V],
      monoid: Monoid[V]
  ): SketchMapMonoid[K, V] =
    new SketchMapMonoid(params)(valueOrdering, monoid)

  def aggregator[K, V](params: SketchMapParams[K])(implicit
      valueOrdering: Ordering[V],
      monoid: Monoid[V]
  ): SketchMapAggregator[K, V] =
...
/** An Aggregator for the SketchMap. Can be created using SketchMap.aggregator
  */
case class SketchMapAggregator[K, V](

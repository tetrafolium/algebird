...
object GenTupleAggregators {
  ...
  def gen(dir: File) = {
    val tupleAggPlace =
      dir / "com" / "twitter" / "algebird" / "GeneratedTupleAggregators.scala"
    IO.write(
      tupleAggPlace,
...
""" + genMethods(22, "implicit def", None) + "\n" + "}"
    ...
    val multiAggPlace =
      dir / "com" / "twitter" / "algebird" / "MultiAggregator.scala"
    IO.write(
      multiAggPlace,
      """package com.twitter.algebird
...
""" +
    ...
    val mapAggPlace =
      dir / "com" / "twitter" / "algebird" / "MapAggregator.scala"
    IO.write(
      mapAggPlace,
      s"""
      |package com.twitter.algebird
  ...
  def genMethods(
    ...
    (2 to max)
      .map(i => {
        ...
        val aggs = nums
          .map(x => "%sAggregator[A, B%s, C%s]".format(aggType, x, x))
          .mkString(", ")
        val prepares =
          nums.map(x => "aggs._%s.prepare(a)".format(x)).mkString(", ")
        val semiType = if (isMonoid) "monoid" else "semigroup"
        val semigroups =
          nums.map(x => "aggs._%s.%s".format(x, semiType)).mkString(", ")
        val semigroup =
          "new Tuple%d%s()(%s)".format(i, semiType.capitalize, semigroups)
        val present =
          nums.map(x => "aggs._%s.present(b._%s)".format(x, x)).mkString(", ")
...
%s %s[A, %s, %s](aggs: Tuple%d[%s]): %sAggregator[A, %s, %s] = {
  ...
  def genMapMethods(max: Int, isMonoid: Boolean = false): String = {
    ...
    val mapAggregatorType =
      if (isMonoid) "MapMonoidAggregator" else "MapAggregator"
    ...
    (2 to max)
      .map(aggrCount => {
        ...
        val inputAggs = aggrNums
          .map(i => s"agg$i: (K, ${inputAggregatorType}[A, B$i, C])")
          .mkString(", ")
      ...
      |      ${aggrNums
            .map(i => s"agg${i}._1 -> agg${i}._2.present(b._${i})")
            .mkString(", ")}
      |    )
      |    def keys: Set[K] = Set(

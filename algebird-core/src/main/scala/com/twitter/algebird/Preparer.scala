...
/** Preparer is a way to build up an Aggregator through composition using a more
  * natural API: it allows you to start with the input type and describe a
  * series of transformations and aggregations from there, rather than starting
  * from the aggregation and composing "outwards" in both directions.
  *
  * Uses of Preparer will always start with a call to Preparer[A], and end with
  * a call to monoidAggregate or a related method, to produce an Aggregator
  * instance.
  */
sealed trait Preparer[A, T] extends java.io.Serializable {

  /** Produce a new MonoidAggregator which includes the Preparer's
    * transformation chain in its prepare stage.
    */
  def monoidAggregate[B, C](
      aggregator: MonoidAggregator[T, B, C]
  ): MonoidAggregator[A, B, C]

  /** Produce a new Preparer that chains this one-to-many transformation.
    * Because "many" could include "none", this limits future aggregations to
    * those done using monoids.
    */
  ...
  /** Filter out values that do not meet the predicate. Like flatMap, this
    * limits future aggregations to MonoidAggregator.
    */
  ...
  /** count and following methods all just call monoidAggregate with one of the
    * standard Aggregators. see the Aggregator object for more docs.
    */
  ...
  /** transform a given Aggregator into a MonoidAggregator by lifting the reduce
    * and present stages into Option space
    */
  def lift[B, C](
      aggregator: Aggregator[T, B, C]
  ): MonoidAggregator[A, Option[B], Option[C]] =
    monoidAggregate(aggregator.lift)

  /** headOption and following methods are all just calling lift with standard
    * Aggregators see the Aggregator object for more docs
    */
...
/** A Preparer that has had zero or more map transformations applied, but no
  * flatMaps. This can produce any type of Aggregator.
  */
trait MapPreparer[A, T] extends Preparer[A, T] {
  ...
  /** Produce a new Aggregator which includes the Preparer's transformation
    * chain in its prepare stage.
    */
  def aggregate[B, C](aggregator: Aggregator[T, B, C]): Aggregator[A, B, C] =
    aggregator.composePrepare(prepareFn)

  /** Split the processing into two parallel aggregations. You provide a
    * function which produces two different aggregators from this preparer, and
    * it will return a single aggregator which does both aggregations in
  ...
  /** head and following methods all just call aggregate with one of the
    * standard Aggregators. see the Aggregator object for more docs.
    */
...
object MapPreparer {
  ...
  /** This is purely an optimization for the case of mapping by identity. It
    * overrides the key methods to not actually use the identity function.
    */
...
/** A Preparer that has had one or more flatMap operations applied. It can only
  * accept MonoidAggregators.
  */
trait FlatMapPreparer[A, T] extends Preparer[A, T] {
  ...
  /** alias of monoidAggregate for convenience unlike MapPreparer's aggregate,
    * can only take MonoidAggregator
    */
  def aggregate[B, C](aggregator: MonoidAggregator[T, B, C]) = monoidAggregate(
    aggregator
  )

  /** Like monoidAggregate, but using an implicit Monoid to construct the
    * Aggregator
    */
  def sum(implicit monoid: Monoid[T]) = monoidAggregate(
    Aggregator.fromMonoid(monoid)
  )
...
object FlatMapPreparer {
  ...
  /** This is purely an optimization for the case of flatMapping by identity. It
    * overrides the key methods to not actually use the identity function.
    */

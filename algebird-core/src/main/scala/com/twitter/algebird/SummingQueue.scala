...
/** A useful utility for aggregation systems: you buffer up some number of items
  ...
  * storm bolts).
  *
  * Without this finite buffer history, an aggregated item could build up
  * infinite history, and therefore it is unbounded in the error you could
  * introduce by losing the buffer.
  *
  * @author
  *   Ashu Singhal
  * @author
...
class SummingQueue[V] private (capacity: Int)(
  ...
  /** puts an item to the queue, optionally sums up the queue and returns value
    * This never blocks interally. It uses offer. If the queue is full, we
    * drain, sum the queue.
    */

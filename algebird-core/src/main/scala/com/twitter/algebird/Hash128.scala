...
/** A typeclass to represent hashing to 128 bits. Used for HLL, but possibly
  * other applications
  */
...
/** This gives default hashes using Murmur128 with a seed of 12345678 (for no
  * good reason, but it should not be changed lest we break serialized HLLs)
  */
object Hash128 extends java.io.Serializable {
  ...
  /** This serializes the string into UTF-8, then hashes. This is different than
    * the UTF-16 based approach in Murmur128.apply(CharSequence), but has been
    * more commonly used in HLL.
    */
  ...
  implicit lazy val stringHash: Hash128[String] = murmur128Utf8String(
    DefaultSeed
  )

...
/** MinHasher as a Monoid operates on this class to avoid the too generic
  * Array[Byte]. The bytes are assumed to be never modified. The only reason we
  * did not use IndexedSeq[Byte] instead of Array[Byte] is because a ByteBuffer
  * is used internally in MinHasher and it can wrap Array[Byte].
  */
case class MinHashSignature(bytes: Array[Byte]) extends AnyVal

object MinHasher {

  /** Numerically solve the inverse of estimatedThreshold, given
    * numBands*numRows
    */
...
/** Instances of MinHasher can create, combine, and compare fixed-sized
  * signatures of arbitrarily sized sets.
  *
  * A signature is represented by a byte array of approx maxBytes size. You can
  * initialize a signature with a single element, usually a Long or String. You
  * can combine any two set's signatures to produce the signature of their
  * union. You can compare any two set's signatures to estimate their Jaccard
  * similarity. You can use a set's signature to estimate the number of distinct
  * values in the set. You can also use a combination of the above to estimate
  * the size of the intersection of two sets from their signatures. The more
  * bytes in the signature, the more accurate all of the above will be.
  *
  * You can also use these signatures to quickly find similar sets without doing
  * n^2 comparisons. Each signature is assigned to several buckets; sets whose
  * signatures end up in the same bucket are likely to be similar. The
  * targetThreshold controls the desired level of similarity - the higher the
  * threshold, the more efficiently you can find all the similar sets.
  *
  * This abstract superclass is generic with regards to the size of the hash
  * used. Depending on the number of unique values in the domain of the sets,
  * you may want a MinHasher16, a MinHasher32, or a new custom subclass.
  *
  * This implementation is modeled after Chapter 3 of Ullman and Rajaraman's
  * Mining of Massive Datasets:
  * http://infolab.stanford.edu/~ullman/mmds/ch3a.pdf
  */
abstract class MinHasher[H](val numHashes: Int, val numBands: Int)(implicit
    n: Numeric[H]
  ...
  /** We always use a 128 bit hash function, so the number of hash functions is
    * different (and usually smaller) than the number of hashes in the
    * signature.
    */
  ...
  /** Bucket keys to use for quickly finding other similar items via locality
    * sensitive hashing
    */
  ...
  /** Maximum value the hash can take on (not 2*hashSize because of signed
    * types)
    */
  def maxHash: H
  ...
  /** Decode two signatures into hash values, combine them somehow, and produce
    * a new array
    */
  protected def buildArray(left: Array[Byte], right: Array[Byte])(
      fn: (H, H) => H
  ): Array[Byte]
}

class MinHasher32(numHashes: Int, numBands: Int)
    extends MinHasher[Int](numHashes, numBands) {
...
class MinHasher16(numHashes: Int, numBands: Int)
    extends MinHasher[Char](numHashes, numBands) {

    ...
    forAll(Gen.choose(2, 100)) { capacity =>
      forAll(Gen.choose(1, 100)) { range =>
        ...
        implicit val ssGenOne: Arbitrary[SSOne[Int]] = Arbitrary {
          for (
            key <- Gen.frequency((1 to range).map { x =>
              (x * x, x: Gen[Int])
            }: _*)
          ) yield SpaceSaver(capacity, key).asInstanceOf[SSOne[Int]]
        }
        implicit def ssGen(implicit
            sg: Semigroup[SpaceSaver[Int]]
        ): Arbitrary[SpaceSaver[Int]] = Arbitrary {
          Gen.oneOf(
            Arbitrary.arbitrary[SSOne[Int]],
            Gen
              .nonEmptyContainerOf[List, SSOne[Int]](
                Arbitrary.arbitrary[SSOne[Int]]
              )
        ...
        commutativeSemigroupLawsEq[SpaceSaver[Int]] { (left, right) =>
  ...
  "SpaceSaver" should {
    "produce a top 20 with exact bounds" in {
      ...
      items
        ...
        .reduce(sg.plus)
        .topK(20)
        .foreach { case (item, approx, guarantee) =>

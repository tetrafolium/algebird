...
object CMSBenchmark {
  ...
  class CMSState {
    ...
    @Param(Array("100"))
    var operations: Int =
      0 // Number of operations per benchmark repetition (cf. `reps`)
...
class CMSBenchmark {
  ...
  def timePlusOfFirstHundredIntegersWithBigIntCms(state: CMSState) = {
    (1 to state.operations).view.foldLeft(state.cmsBigIntMonoid.zero)(
      (l, r) => { l ++ state.cmsBigIntMonoid.create(r) }
    )
  }

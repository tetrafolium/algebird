...
class RightFoldedTest extends CheckProperties {
  ...
  implicit def rightFoldedValue[Out: Arbitrary]
      : Arbitrary[RightFoldedValue[Out]] =
    Arbitrary {
      for (v <- implicitly[Arbitrary[Out]].arbitrary) yield RightFoldedValue(v)
    }
  implicit def rightFoldedToFold[In: Arbitrary]
      : Arbitrary[RightFoldedToFold[In]] =
    Arbitrary {
      for (v <- implicitly[Arbitrary[In]].arbitrary)
        yield RightFoldedToFold(List(v))
    }
  implicit def rightFolded[In: Arbitrary, Out: Arbitrary]
      : Arbitrary[RightFolded[In, Out]] =
    Arbitrary {
      Gen.oneOf(
        rightFoldedValue[Out].arbitrary,
        rightFoldedToFold[In].arbitrary
      )
    }

...
/** import com.twitter.algebird.spark.ToAlgebird to get the enrichment to do:
...
class AlgebirdRDD[T](val rdd: RDD[T]) extends AnyVal {
  ...
  /** Apply an Aggregator to return a single value for the whole RDD. If the RDD
    * is empty, None is returned requires a commutative Semigroup. To generalize
    * to non-commutative, we need a sorted partition for T.
    */
  ...
  /** This will throw if you use a non-MonoidAggregator with an empty RDD
    * requires a commutative Semigroup. To generalize to non-commutative, we
  ...
  /** Apply an Aggregator to the values for each key. requires a commutative
    * Semigroup. To generalize to non-commutative, we need a sorted partition
    * for T.
    */
  def aggregateByKey[K: ClassTag, V1, U: ClassTag, V2](
      ...
      ev: T <:< (K, V1),
      ordK: Priority[Ordering[K], DummyImplicit]
  ): RDD[(K, V2)] =
    aggregateByKey(Partitioner.defaultPartitioner(rdd), agg)

  private def toPair[K: ClassTag, V: ClassTag](kv: RDD[(K, V)])(implicit
      ordK: Priority[Ordering[K], DummyImplicit]
  ) =
  ...
  def aggregateByKey[K: ClassTag, V1, U: ClassTag, V2](
     ...
     */
    val prepared = keyed.mapPartitions(
      { it =>
  ...
  private def keyed[K, V](implicit ev: T <:< (K, V)): RDD[(K, V)] =
    rdd.asInstanceOf[RDD[(K, V)]]

  /** Use the implicit semigroup to sum by keys requires a commutative
    * Semigroup. To generalize to non-commutative, we need a sorted partition
    * for T.
    */
  ...
  /** Use the implicit semigroup to sum by keys with a custom Partitioner.
    * requires a commutative Semigroup. To generalize to non-commutative, we
    * need a sorted partition for T. Unfortunately we need to use a different
    * name than sumByKey in scala 2.11
    */
  def sumByKey[K: ClassTag, V: ClassTag: Semigroup](part: Partitioner)(implicit
      ev: T <:< (K, V),
      ord: Priority[Ordering[K], DummyImplicit]
  ): RDD[(K, V)] =
    toPair(keyed).reduceByKey(part, implicitly[Semigroup[V]].plus _)

  /** Use the implicit Monoid to sum all items. If RDD is empty, Monoid.zero is
    * returned requires a commutative Monoid. To generalize to non-commutative,
    * we need a sorted partition for T.
    */
  def sum(implicit mon: Monoid[T], ct: ClassTag[T]): T =
    sumOption.getOrElse(mon.zero)

  /** Use the implicit Semigroup to sum all items. If there are no items, None
    * is returned. requires a commutative Monoid. To generalize to

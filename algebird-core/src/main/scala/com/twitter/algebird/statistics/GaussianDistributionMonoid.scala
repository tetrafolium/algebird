...
/** This monoid stems from the fact that if X and Y are independent random
  * variables that are normally distributed, then their sum is also normally
  * distributed, with its new mean equal to the sum of two means and variance
  * equal to the sum of two variances.
...
object GaussianDistributionMonoid extends Monoid[GaussianDistribution] {
  ...
  override def sumOption(
      its: TraversableOnce[GaussianDistribution]

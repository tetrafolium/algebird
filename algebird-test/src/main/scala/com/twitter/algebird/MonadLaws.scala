...
object MonadLaws {
  ...
  def leftIdentity[M[_], T, U](eq: (M[U], M[U]) => Boolean = defaultEq[M[U]])(
    ...
    forAll { (t: T, fn: T => M[U]) =>
      eq(t.pure[M].flatMap(fn), fn(t))
    }

  def rightIdentity[M[_], T](
      eq: (M[T], M[T]) => Boolean = defaultEq[M[T]]
  )(implicit monad: Monad[M], arb: Arbitrary[M[T]]) =
    forAll { (mt: M[T]) =>
      eq(mt.flatMap { _.pure[M] }, mt)
    }
  ...
  def associative[M[_], T, U, V](eq: (M[V], M[V]) => Boolean)(
    ...
    eq(mt.flatMap(f1).flatMap(f2), mt.flatMap { t =>
      f1(t).flatMap(f2)
    })
  }
  ...
  implicit def fnArb[M[_], T, U](
      implicit map: Arbitrary[Map[T, M[U]]],
      arbu: Arbitrary[M[U]]
  ): Arbitrary[T => M[U]] = Arbitrary {
    for (m <- implicitly[Arbitrary[Map[T, M[U]]]].arbitrary;
      defu <- implicitly[Arbitrary[M[U]]].arbitrary) yield ({ (t: T) =>
      m.getOrElse(t, defu)
    })
  }
  ...
  def monadLaws[M[_], T, U, R](eq: (M[R], M[R]) => Boolean = defaultEq[M[R]])(
    ...
    associative[M, T, U, R](eq) && rightIdentity[M, R](eq) && leftIdentity[
      M,
      U,
      R
    ](eq)

  implicit def indexedSeqA[T](
      implicit arbl: Arbitrary[List[T]]
  ): Arbitrary[IndexedSeq[T]] =
    Arbitrary { arbl.arbitrary.map { _.toIndexedSeq } }

  implicit def vectorA[T](
      implicit arbl: Arbitrary[List[T]]
  ): Arbitrary[Vector[T]] =
    Arbitrary {

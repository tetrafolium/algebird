...
/** Aggregators compose well.
  *
  * To create a parallel aggregator that operates on a single input in parallel,
  * use: GeneratedTupleAggregator.from2((agg1, agg2))
  */
object Aggregator extends java.io.Serializable {

  implicit def applicative[I]
      : Applicative[({ type L[O] = Aggregator[I, _, O] })#L] =
  ...
  /** Using Aggregator.prepare,present you can add to this aggregator
    */
  def fromReduce[T](red: (T, T) => T): Aggregator[T, T, T] = fromSemigroup(
    Semigroup.from(red)
  )
  ...
  def fromMonoid[F, T](implicit
      mon: Monoid[T],
      prep: F => T
  ): MonoidAggregator[F, T, T] =
    prepareMonoid(prep)(mon)

  def prepareMonoid[F, T](prep: F => T)(implicit
      m: Monoid[T]
  ): MonoidAggregator[F, T, T] = new MonoidAggregator[F, T, T] {
    def prepare(input: F) = prep(input)
    def monoid = m
    def present(reduction: T) = reduction
  }
  // Uses the product from the ring
  def fromRing[F, T](implicit
      rng: Ring[T],
      prep: F => T
  ): RingAggregator[F, T, T] = new RingAggregator[F, T, T] {
    def prepare(input: F) = prep(input)
    def ring = rng
    def present(reduction: T) = reduction
  }
  ...
  /** Take the smallest `count` items using a heap
    */
  def sortedTake[T: Ordering](
      count: Int
  ): MonoidAggregator[T, PriorityQueue[T], Seq[T]] =
    new mutable.PriorityQueueToListAggregator[T](count)

  /** Take the largest `count` items using a heap
    */
  def sortedReverseTake[T: Ordering](
      count: Int
  ): MonoidAggregator[T, PriorityQueue[T], Seq[T]] =
    new mutable.PriorityQueueToListAggregator[T](count)(
  ...
  /** Immutable version of sortedTake, for frameworks that check immutability of
    * reduce functions.
    */
  def immutableSortedTake[T: Ordering](
      count: Int
  ): MonoidAggregator[T, TopK[T], Seq[T]] =
    new TopKToListAggregator[T](count)

  /** Immutable version of sortedReverseTake, for frameworks that check
    * immutability of reduce functions.
    */
  def immutableSortedReverseTake[T: Ordering](
      count: Int
  ): MonoidAggregator[T, TopK[T], Seq[T]] =
    new TopKToListAggregator[T](count)(implicitly[Ordering[T]].reverse)

  /** Put everything in a List. Note, this could fill the memory if the List is
    * very large.
    */
  def toList[T]: MonoidAggregator[T, List[T], List[T]] =
    prepareMonoid { t: T => List(t) }

  /** Put everything in a Set. Note, this could fill the memory if the Set is
    * very large.
    */
  ...
  /** This builds an in-memory Set, and then finally gets the size of that set.
    * This may not be scalable if the Uniques are very large. You might check
    * the approximateUniqueCount or HyperLogLog Aggregator to get an approximate
    * version of this that is scalable.
    */
  def uniqueCount[T]: MonoidAggregator[T, Set[T], Int] =
    toSet[T].andThenPresent(_.size)

  /** Using a constant amount of memory, give an approximate unique count (~ 1%
    * error). This uses an exact set for up to 100 items, then HyperLogLog (HLL)
    * with an 1.2% standard error which uses at most 8192 bytes for each HLL.
    * For more control, see HyperLogLogAggregator.
    */
  def approximateUniqueCount[T: Hash128]
      : MonoidAggregator[T, Either[HLL, Set[T]], Long] =
    SetSizeHashAggregator[T](hllBits = 13, maxSetSize = 100)

  /** Returns the lower bound of a given percentile where the percentile is
    * between (0,1] The items that are iterated over cannot be negative.
    */
  def approximatePercentile[T](percentile: Double, k: Int)(implicit
      num: Numeric[T]
  ): QTreeAggregatorLowerBound[T] =
    QTreeAggregatorLowerBound[T](percentile, k)

  /** Returns the intersection of a bounded percentile where the percentile is
    * between (0,1] The items that are iterated over cannot be negative.
    */
  def approximatePercentileBounds[T](percentile: Double, k: Int)(implicit
      num: Numeric[T]
  ): QTreeAggregator[T] =
    QTreeAggregator[T](percentile, k)
}

/** This is a type that models map/reduce(map). First each item is mapped, then
  * we reduce with a semigroup, then finally we present the results.
  *
  * Unlike Fold, Aggregator keeps it's middle aggregation type externally
  * visible. This is because Aggregators are useful in parallel map/reduce
  * systems where there may be some additional types needed to cross the
  * map/reduce boundary (such a serialization and intermediate storage). If you
  * don't care about the middle type, an _ may be used and the main utility of
  * the instance is still preserved (e.g. def operate[T](ag: Aggregator[T, _,
  * Int]): Int)
  *
...
trait Aggregator[-A, B, +C] extends java.io.Serializable { self =>
  ...
  /** This may error if items is empty. To be safe you might use reduceOption if
    * you don't know that items is non-empty
    */
  ...
  /** This may error if inputs are empty (for Monoid Aggregators it never will,
    * instead you see present(Monoid.zero[B])
    */
  def apply(inputs: TraversableOnce[A]): C = present(
    reduce(inputs.map(prepare))
  )
  ...
  /** This returns the cumulative sum of its inputs, in the same order. If the
    * inputs are empty, the result will be empty too.
    */
  ...
  /** This allows you to run two aggregators on the same data with a single pass
    */
  def join[A2 <: A, B2, C2](
      that: Aggregator[A2, B2, C2]
  ): Aggregator[A2, (B, B2), (C, C2)] =
    GeneratedTupleAggregator.from2((this, that))

  /** This allows you to join two aggregators into one that takes a tuple input,
    * which in turn allows you to chain .composePrepare onto the result if you
    * have an initial input that has to be prepared differently for each of the
    * joined aggregators.
    *
    * The law here is: ag1.zip(ag2).apply(as.zip(bs)) == (ag1(as), ag2(bs))
    */
  def zip[A2, B2, C2](
      ag2: Aggregator[A2, B2, C2]
  ): Aggregator[(A, A2), (B, B2), (C, C2)] = {
    val ag1 = this
  ...
  /** An Aggregator can be converted to a Fold, but not vice-versa Note, a Fold
    * is more constrained so only do this if you require joining a Fold with an
    * Aggregator to produce a Fold
    */
...
/** Aggregators are Applicatives, but this hides the middle type. If you need a
  * join that does not hide the middle type use join on the trait, or
  * GeneratedTupleAggregator.fromN
  */
class AggregatorApplicative[I]
    extends Applicative[({ type L[O] = Aggregator[I, _, O] })#L] {

  override def map[T, U](mt: Aggregator[I, _, T])(
      fn: T => U
  ): Aggregator[I, _, U] =
...
trait MonoidAggregator[-A, B, +C] extends Aggregator[A, B, C] { self =>
  ...
  override def andThenPresent[D](
      present2: C => D
  ): MonoidAggregator[A, B, D] = {
    val self = this
  ...
  override def composePrepare[A2](
      prepare2: A2 => A
  ): MonoidAggregator[A2, B, C] = {
    val self = this
  ...
  /** Build a MonoidAggregator that either takes left or right input and outputs
    * the pair from both
    */
  def either[A2, B2, C2](
  ...
  /** This maps the inputs to Bs, then sums them, effectively flattening the
    * inputs to the MonoidAggregator
    */
  ...
  /** This allows you to join two aggregators into one that takes a tuple input,
    ...
    */
  def zip[A2, B2, C2](
      ag2: MonoidAggregator[A2, B2, C2]
  ): MonoidAggregator[(A, A2), (B, B2), (C, C2)] = {
    val ag1 = self

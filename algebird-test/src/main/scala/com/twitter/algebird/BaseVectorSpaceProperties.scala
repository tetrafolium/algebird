...
object BaseVectorSpaceProperties {
  ...
  def vectorSpaceLaws[F, C[_]](
    ...
    isEqualIfZero(eqfn) && distributesWithPlus(eqfn) && isAssociative(
      eqfn
    ) && identityOne(eqfn) && distributesOverScalarPlus(eqfn)

  def beCloseTo(a: Double, b: Double) =
    a == b || (math.abs(a - b) / math.abs(
      a
    )) < 1e-10 || (a.isInfinite && b.isInfinite) || a.isNaN || b.isNaN
}

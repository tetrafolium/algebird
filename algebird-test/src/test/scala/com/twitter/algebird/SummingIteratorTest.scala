...
class SummingIteratorTest extends PropSpec with PropertyChecks with Matchers {
  ...
  def sumEquiv[V: Semigroup: Equiv](
      it0: Iterator[V],
      it1: Iterator[V]
  ): Boolean =
    StatefulSummerLaws.zeroEquiv(
      Semigroup.sumOption(it0),
      Semigroup.sumOption(it1)
    )
  ...
  property("With Maps is preserved[(Short,Int)]") {
    forAll { (cap: Capacity, items: List[(Short, Int)]) =>
      ...
      val qit = SummingIterator[Map[Short, Int]](
        SummingQueue[Map[Short, Int]](cap.c),
        mitems.iterator
      )
      val qitc = SummingIterator[Map[Short, Int]](
        SummingCache[Short, Int](cap.c),
        mitems.iterator
      )
  ...
  property("With Maps is preserved[(Short,String)]") {
    forAll { (cap: Capacity, items: List[(Short, String)]) =>
      val mitems = items.map { Map(_) }
      val qit = SummingIterator(
        SummingQueue[Map[Short, String]](cap.c),
        mitems.iterator
      )
      val qitc =

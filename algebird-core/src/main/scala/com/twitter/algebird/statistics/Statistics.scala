...
/** These wrappers can be used to collect statistics around usage of monoids
  * They are thread safe unless false is passed to the constructor (to remove
  * overhead when threads are not used)
  *
  * @author
  *   Julien Le Dem
  */
...
class StatisticsSemigroup[T](threadSafe: Boolean = true)(implicit
    wrappedSemigroup: Semigroup[T]
) extends Semigroup[T] {

  private[this] final val plusCallsCount = Counter(threadSafe)
  private[this] final val sumOptionCallsStats = new IterCallStatistics(
    threadSafe
  )
...
/** @see
  *   StatisticsSemigroup
  */
class StatisticsMonoid[T](threadSafe: Boolean = true)(implicit
    wrappedMonoid: Monoid[T]

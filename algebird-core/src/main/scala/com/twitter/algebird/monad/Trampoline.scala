...
final case class FlatMapped[C, A](start: Trampoline[C], fn: C => Trampoline[A])
...
object Trampoline {
  ...
  def lazyVal[A](a: => A): Trampoline[A] =
    FlatMapped(unit, { (u: Unit) => Done(a) })

  /** Use this to call to another trampoline returning function you break the
    * effect of this if you directly recursively call a Trampoline returning
    * function
    */
  def call[A](layzee: => Trampoline[A]): Trampoline[A] =
    FlatMapped(unit, { (u: Unit) => layzee })
  ...
  def run[A](tramp: Trampoline[A]): A = {
    @annotation.tailrec
    def loop(
        start: Trampoline[Any],
        stack: List[(Any) => Trampoline[Any]]
    ): Any = {
      start match {
        case Done(a) =>
          stack match {

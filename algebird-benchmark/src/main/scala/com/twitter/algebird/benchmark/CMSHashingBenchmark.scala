...
/** Benchmarks the hashing algorithms used by Count-Min sketch for CMS[BigInt].
  *
  * The input values are generated ahead of time to ensure that each trial uses
  * the same input (and that the RNG is not influencing the runtime of the
  * trials).
  *
...
object CMSHashingBenchmark {
  ...
  class CMSState {
    ...
    /** The `a` parameter for CMS' default ("legacy") hashing algorithm:
      * `h_i(x) = a_i * x + b_i (mod p)`.
      */
    @Param(Array("5123456"))
    var a: Int = 0

    /** The `b` parameter for CMS' default ("legacy") hashing algorithm:
      * `h_i(x) = a_i * x + b_i (mod p)`.
      *

...
case class BitSetLite(in: Array[Byte]) {
  def contains(x: Int): Boolean = {
    ...
    /** Pretend 'in' is little endian so that the bitstring b0b1b2b3 is such
      * that if b0 == 1, then 0 is in the bitset, if b1 == 1, then 1 is in the
      * bitset.
      */
    val arrayIdx = x / 8
    val remainder = x % 8
...
object HyperLogLog {
  ...
  /** the value 'j' is equal to <w_0, w_1 ... w_(bits-1)> TODO: We could read in
    * a byte at a time.
    */
  ...
  /** The value 'w' is equal to <w_bits ... w_n>. The function rho counts the
    * number of leading zeroes in 'w'. We can calculate rho(w) at once with the
    * method rhoW.
    */
  ...
  /** We are computing j and \rho(w) from the paper, sorry for the name, but it
    * allows someone to compare to the paper extremely low probability rhow
    * (position of the leftmost one bit) is > 127, so we use a Byte to store it
    * Given a hash <w_0, w_1, w_2 ... w_n> the value 'j' is equal to <w_0, w_1
    * ... w_(bits-1)> and the value 'w' is equal to <w_bits ... w_n>. The
    * function rho counts the number of leading zeroes in 'w'. We can calculate
    * rho(w) at once with the method rhoW.
    */
  ...
  /** The true error is distributed like a Gaussian with this standard
    * deviation. let m = 2^bits. The size of the HLL is m bytes.
    *
    * bits | size | error 9 512 0.0460 10 1024 0.0325 11 2048 0.0230 12 4096
    * 0.0163 13 8192 0.0115 14 16384 0.0081 15 32768 0.0057 16 65536 0.0041 17
    * 131072 0.0029 18 262144 0.0020 19 524288 0.0014 20 1048576 0.0010
    *
    * Keep in mind, to store N distinct longs, you only need 8N bytes. See
    * SetSizeAggregator for an approach that uses an exact set when the
    * cardinality is small, and switches to HLL after we have enough items.
    * Ideally, you would keep an exact set until it is smaller to store the HLL
    * (but actually since we use sparse vectors to store the HLL, a small HLL
    * takes a lot less than the size above).
    */
  ...
  def bitsForError(err: Double): Int = {
    ...
    require(
      err >= 0.00003 && err < 1.0,
      s"Error must be in (0.00003, 1.0): $err"
    )
...
sealed abstract class HLL extends java.io.Serializable {
  ...
  /** Returns the modified value of rhoW at j, taking into account the extra run
    * of bits added to rho due to reduction in the length of j.
    *
    * @param currentJ
    ...
    * @param bitMask
    *   Mask to force early termination of HyperLogLog.rhoW (passed in to avoid
  ...
  def downsize(reducedBits: Int): HLL = {
    require(
      reducedBits > 3 && reducedBits <= bits,
      s"Use at least 4, and at most $bits bits"
    )
    if (reducedBits == bits)
      this
    else {
...
case class SparseHLL(bits: Int, maxRhow: Map[Int, Max[Byte]]) extends HLL {
  ...
  def +(other: HLL) = {
    ...
    other match {
      ...
      case sparse @ SparseHLL(_, oMaxRhow) =>
        assert(
          sparse.size == size,
          "Incompatible HLL size: " + sparse.size + " != " + size
        )
      ...
      case DenseHLL(bits, oldV) =>
        assert(
          oldV.size == size,
          "Incompatible HLL size: " + oldV.size + " != " + size
        )
  ...
  override protected def downsize(
      reducedBits: Int,
      reducedSize: Int,
      bitMask: Int,
      buf: Array[Byte]
...
case class DenseHLL(bits: Int, v: Bytes) extends HLL {
  ...
  assert(
    v.size == (1 << bits),
    "Invalid size for dense vector: " + size + " != (1 << " + bits + ")"
  )
  ...
  def +(other: HLL): HLL = {
    ...
    other match {
      ...
      case DenseHLL(_, ov) =>
        assert(
          ov.size == v.size,
          "Incompatible HLL size: " + ov.size + " != " + v.size
        )
...
class HyperLogLogMonoid(val bits: Int) extends Monoid[HLL] {
  ...
  private[this] final def denseUpdate(
      existing: HLL,
      iter: Iterator[HLL]
  ): HLL = {
...
object HyperLogLogAggregator {
  ...
  /** Create an Aggregator that returns the estimate size, not the HLL
    * approximate data structure itself. This is convenient, but cannot be
    * combined later with another unique count like an HLL could.
    *
    * @param bits
  ...
  /** Give a HyperLogLog Aggregator that have the given error. It is up to you,
    * using bitsForError, to see if the size is still practical for your
    * application.
    *
    * 0.016 (1.6%), 4 KB 0.006 (0.6%), 32 KB 0.002 (0.2%), 256 KB 0.001 (0.1%),
    * 1024 KB
    *
  ...
  def withBits[K](
      bits: Int
  )(implicit hash128: Hash128[K]): GenHLLAggregator[K] = {
    val monoid = new HyperLogLogMonoid(bits)
    GenHLLAggregator(monoid, hash128)
  }

  /** Give an approximate set size (not the HLL) based on inputs of Array[Byte]
    * see HyperLogLog.bitsForError for a size table based on the error see
    * SetSizeHashAggregator for a version that uses exact sets up to a given
    * size
    */
  def sizeWithError(err: Double): MonoidAggregator[Array[Byte], HLL, Double] =
    withError(err).andThenPresent(_.estimatedSize)

  def sizeWithErrorGeneric[K](err: Double)(implicit
      hash128: Hash128[K]
  ): MonoidAggregator[K, HLL, Long] =
    withErrorGeneric(err).andThenPresent(_.estimatedSize.toLong)
}

case class GenHLLAggregator[K](
...
case class SetSizeAggregator[A](hllBits: Int, maxSetSize: Int = 10)(implicit
...
/** Use a Hash128 when converting to HLL, rather than an implicit conversion to
  * Array[Byte] Unifying with SetSizeAggregator would be nice, but since they
  * only differ in an implicit parameter, scala seems to be giving me errors.
  */
case class SetSizeHashAggregator[A](hllBits: Int, maxSetSize: Int = 10)(implicit
    hash: Hash128[A]
) extends SetSizeAggregatorBase[A](hllBits, maxSetSize) {
  def convert(set: Set[A]) = leftSemigroup.sum(set.iterator.map { a =>

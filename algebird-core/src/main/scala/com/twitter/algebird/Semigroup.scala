...
/** Semigroup: This is a class with a plus method that is associative: a+(b+c) =
  * (a+b)+c
  */
...
trait Semigroup[@specialized(Int, Long, Float, Double) T]
  ...
  /** override this if there is a faster way to do this sum than
    * reduceLeftOption on plus
    */
...
/** Either semigroup is useful for error handling. if everything is correct, use
  * Right (it's right, get it?), if something goes wrong, use Left. plus does
  * the normal thing for plus(Right, Right), or plus(Left, Left), but if exactly
  * one is Left, we return that value (to keep the error condition). Typically,
  * the left value will be a string representing the errors.
  */
class EitherSemigroup[L, R](implicit
...
object Semigroup extends GeneratedSemigroupImplicits with ProductSemigroups {
  ...
  // Left sum: (((a + b) + c) + d)
  def sumOption[T](iter: TraversableOnce[T])(implicit
      sg: Semigroup[T]
  ): Option[T] =
    sg.sumOption(iter)
  ...
  /** Same as v + v + v .. + v (i times in total) requires i > 0, wish we had
    * PositiveBigInt as a class
    */
  ...
  private def timesPow2[T](power: Int, v: T, memo: Vector[T])(implicit
      sg: Semigroup[T]
  ): (T, Vector[T]) = {
    val size = memo.size

...
/** Folds are first-class representations of "Traversable.foldLeft." They have
  * the nice property that they can be fused to work in parallel over an input
  * sequence.
  *
  * A Fold accumulates inputs (I) into some internal type (X), converting to a
  * defined output type (O) when done. We use existential types to hide internal
  * details and to allow for internal and external (X and O) types to differ for
  * "map" and "join."
  *
  * In discussing this type we draw parallels to Function1 and related types.
  * You can think of a fold as a function "Seq[I] => O" but in reality we do not
  * have to materialize the input sequence at once to "run" the fold.
  *
  * The traversal of the input data structure is NOT done by Fold itself.
  * Instead we expose some methods like "overTraversable" that know how to
  * iterate through various sequence types and drive the fold. We also expose
  * some internal state so library authors can fold over their own types.
  *
...
sealed trait Fold[-I, +O] extends Serializable {
  ...
  /** Users can ignore this type.
    *
    * The internal accumulator type. No one outside this Fold needs to know what
    * this is, and that's a good thing. It keeps type signatures sane and makes
    * this easy to use for the amount of flexibility it provides.
    */
  type X

  /** Users can ignore this method. It is exposed so library authors can run
    * folds over their own sequence types.
    *
    * "build" constructs a FoldState, which tells us how to run the fold. It is
    * expected that we can run the same Fold many times over different data
    * structures, but we must build a new FoldState every time.
    *
    * See FoldState for information on how to use this for your own sequence
    * types.
    */
  def build(): FoldState[X, I, O]

  /** Transforms the output of the Fold after iteration is complete. This is
    * analogous to "Future.map" or "Function1.compose."
    */
  ...
  /** Joins two folds into one and combines the results. The fused fold
    * accumulates with both at the same time and combines at the end.
    */
  def joinWith[I2 <: I, P, Q](
      other: Fold[I2, P]
  )(f: (O, P) => Q): Fold[I2, Q] = {
    val self = this
    new Fold[I2, Q] {
  ...
  /** Transforms the input of the fold before every accumulation. (The name
    * comes from "contravariant map.") This is analogous to "Function1.andThen."
    */
...
/** A FoldState defines a left fold with a "hidden" accumulator type. It is
  * exposed so library authors can run Folds over their own sequence types.
  *
  * The fold can be executed correctly according to the properties of "add" and
  * your traversed data structure. For example, the "add" function of a monoidal
  * fold will be associative. A FoldState is valid for only one iteration
  * because the accumulator (seeded by "start" may be mutable.
  *
  * The three components of a fold are add: (X, I) => X - updates and returns
  * internal state for every input I start: X - the initial state end: X => O -
  * transforms internal state to a final result
  *
...
/** Methods to create and run Folds.
  *
  * The Folds defined here are immutable and serializable, which we expect by
  * default. It is important that you as a user indicate mutability or
  * non-serializability when defining new Folds. Additionally, it is recommended
  * that "end" functions not mutate the accumulator in order to support scans
  * (producing a stream of intermediate outputs by calling "end" at each step).
  */
object Fold {
  ...
  /** Turn a common Scala foldLeft into a Fold. The accumulator MUST be
  ...
  /** A general way of defining Folds that supports constructing mutable or
    * non-serializable accumulators.
    */
  def foldMutable[M, I, O](
      add: (M, I) => M,
      start: Unit => M,
      end: M => O
  ): Fold[I, O] =
  ...
  /** Simple Fold that collects elements into a container.
    */
  def container[I, C[_]](implicit
      cbf: CanBuildFrom[C[I], I, C[I]]
  ): Fold[I, C[I]] =
  ...
  /** An even simpler Fold that collects into a Seq. Shorthand for "container[I,
    * Seq];" fewer type arguments, better type inferrence.
    */
  def seq[I]: Fold[I, Seq[I]] =
    container[I, Seq]

  /** A Fold that does no work and returns a constant. Analogous to Function1
    * const: def const[A, B](b: B): (A => B) = { _ => b }
    */
  ...
  /** A Fold that returns the max value in a sequence. (Biased to earlier equal
    * values.)
    */
  ...
  /** A Fold that returns a min value in a sequence. (Biased to earlier equal
    * values.)
    */
  ...
  /** A Fold that returns the sum of a numeric sequence. Does not protect
    * against overflow.
    */
  ...
  /** A Fold that returns the product of a numeric sequence. Does not protect
    * against overflow.
    */
  ...
  /** A Fold that returns "true" if all elements of the sequence statisfy the
    * predicate. Note this does not short-circuit enumeration of the sequence.
    */
  def forall[I](pred: I => Boolean): Fold[I, Boolean] =
    foldLeft(true) { (b, i) => b && pred(i) }

  /** A Fold that returns "true" if any element of the sequence statisfies the
    * predicate. Note this does not short-circuit enumeration of the sequence.
    */
...
class FoldApplicative[I] extends Applicative[({ type L[O] = Fold[I, O] })#L] {
  ...
  override def sequence[T](ms: Seq[Fold[I, T]]): Fold[I, Seq[T]] =
    Fold.sequence(ms)
  override def joinWith[T, U, V](mt: Fold[I, T], mu: Fold[I, U])(
      fn: (T, U) => V
  ): Fold[I, V] =
    mt.joinWith(mu)(fn)
}

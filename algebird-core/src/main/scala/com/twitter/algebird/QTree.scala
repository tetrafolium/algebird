...
class QTree[@specialized(Int, Long, Float, Double) A] private[algebird] (
...
) extends scala.Product6[Long, Int, Long, A, Option[QTree[A]], Option[
      QTree[A]
    ]]
    with Serializable {
  import QTree._
  ...
  /**
    ...
    */
  def rangeSumBounds(from: Double, to: Double)(
      implicit monoid: Monoid[A]
  ): (A, A) = {
  ...
  @inline
  private def pruneChild(minCount: Long, childNullable: QTree[A])(
      implicit m: Monoid[A]
  ): QTree[A] =
...
case class QTreeAggregator[T](
    ...
    extends Aggregator[T, QTree[Unit], Intersection[
      InclusiveLower,
      InclusiveUpper,
      Double
    ]]

...
sealed trait Interval[T] extends java.io.Serializable {
  ...
  /** Map the Interval with a non-decreasing function. If you use a
    * non-monotonic function (like x^2) then the result is meaningless. TODO: It
    * might be good to have types for these properties in algebird.
    */
...
object Interval extends java.io.Serializable {
  ...
  implicit def monoid[T]: Monoid[Interval[T]] =
    Monoid.from[Interval[T]](Universe[T]()) { _ && _ }

  // Automatically convert from an either
  implicit def fromEither[L[t] <: Interval[t], R[t] <: Interval[t], T](
      either: Either[L[T], R[T]]
  ): Interval[T] =
    either match {
      case Right(i) => i
      case Left(i)  => i
...
sealed trait Lower[T] extends Interval[T] {
  ...
  /** This may give a false positive (but should try not to). Note the case of
    * (0,1) for the integers. If they were doubles, this would intersect, but
    * since there are no members of the set Int that are bigger than 0 and less
    * than 1, they don't really intersect. So, ordering is not enough here. You
    * need a stronger notion, which we don't have a typeclass for.
    */
  def intersects(u: Upper[T]): Boolean

  /** The smallest value that is contained here This is an Option, because of
    * cases like ExclusiveLower(Int.MaxValue) which are pathological and
    * equivalent to Empty
    */
...
sealed trait Upper[T] extends Interval[T] {
  ...
  /** The smallest value that is contained here This is an Option, because of
    * cases like ExclusiveUpper(Int.MinValue), which are pathological and
    * equivalent to Empty
    */
...
case class InclusiveLower[T](lower: T)(implicit val ordering: Ordering[T])
    extends Interval[T]
    with Lower[T] {
  ...
  def intersect(that: Interval[T]): Interval[T] = that match {
    ...
    case ub @ ExclusiveUpper(upper) =>
      if (intersects(ub)) Intersection(this, ub) else Empty()
    case lb @ InclusiveLower(thatlb) =>
      if (lb.ordering.gt(lower, thatlb)) this else that
    case lb @ ExclusiveLower(thatlb) =>
      if (lb.ordering.gt(lower, thatlb)) this else that
  ...
  def mapNonDecreasing[U: Ordering](fn: T => U): Interval[U] = InclusiveLower(
    fn(lower)
  )
}
case class ExclusiveLower[T](lower: T)(implicit val ordering: Ordering[T])
    extends Interval[T]
    with Lower[T] {
  ...
  def intersect(that: Interval[T]): Interval[T] = that match {
    ...
    case ub @ ExclusiveUpper(upper) =>
      if (intersects(ub)) Intersection(this, ub) else Empty()
    case lb @ InclusiveLower(thatlb) =>
      if (lb.ordering.gteq(lower, thatlb)) this else that
    case lb @ ExclusiveLower(thatlb) =>
      if (lb.ordering.gteq(lower, thatlb)) this else that
    case Intersection(thatL, thatU) => (this && thatL) && thatU
  }
  ...
  def mapNonDecreasing[U: Ordering](fn: T => U): Interval[U] = ExclusiveLower(
    fn(lower)
  )
}
case class InclusiveUpper[T](upper: T)(implicit val ordering: Ordering[T])
    extends Interval[T]
    with Upper[T] {
  ...
  def mapNonDecreasing[U: Ordering](fn: T => U): Interval[U] = InclusiveUpper(
    fn(upper)
  )
}
case class ExclusiveUpper[T](upper: T)(implicit val ordering: Ordering[T])
    extends Interval[T]
    with Upper[T] {
  ...
  def mapNonDecreasing[U: Ordering](fn: T => U): Interval[U] = ExclusiveUpper(
    fn(upper)
  )
}

case class Intersection[L[t] <: Lower[t], U[t] <: Upper[t], T](
    lower: L[T],
    upper: U[T]
) extends Interval[T] {
  ...
  require(
    ...
    "Intersections must be non-empty: (%s, %s)".format(lower, upper)
  )
  ...
  /** Goes from lowest to highest for all items that are contained in this
    * Intersection
    */
  ...
  /** Goes from highest to lowest for all items that are contained in this
    * Intersection
    */
  ...
  /** Some intervals can actually be synonyms for empty: (0,0) for instance,
    * contains nothing. This cannot be normalized to [a, b) form, thus we return
    * an option Also, there are cases like [Int.MinValue, Int.MaxValue] that
    * cannot are actually equivalent to Universe. The bottom line: if this
    * returns None, it just means you can't express it this way, it does not
    * mean it is empty or universe, etc... (there are other cases).
    */

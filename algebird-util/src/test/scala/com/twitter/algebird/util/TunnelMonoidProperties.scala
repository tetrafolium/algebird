...
object TunnelMonoidProperties {
  def testTunnelMonoid[I, V](
  ...
  )(implicit monoid: Monoid[I], superMonoid: Monoid[V]) = {
    val r = new Random
    ...
    numbers.forall { _ =>
      ...
      val finalResults =
        helper(numbers, toFeed) zip helper(numbers, toFeed) map {
          case (f1, f2) =>
            for {
              b1 <- f1
              b2 <- f2
...
class TunnelMonoidPropertiesextends extends CheckProperties {
  ...
  property("associative") {
    ...
    def collapseFinalValues(
        finalTunnel: Tunnel[Int],
        tunnels: Seq[Tunnel[Int]],
        toFeed: Int
    ) =
      finalTunnel(toFeed) +: tunnels.map { _.future }

    testTunnelMonoid[Int, Tunnel[Int]](
      identity,
      makeTunnel,
      collapseFinalValues
    )

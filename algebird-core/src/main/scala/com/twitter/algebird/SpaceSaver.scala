...
object SpaceSaver {
  ...
  /** Construct SpaceSaver with given capacity containing a single item. This is
    * the public api to create a new SpaceSaver.
    */
  def apply[T](capacity: Int, item: T): SpaceSaver[T] = SSOne(capacity, item)

  private[algebird] val ordering =
    Ordering.by[(_, (Long, Long)), (Long, Long)] { case (item, (count, err)) =>
      (-count, err)
    }
...
/** Data structure used in the Space-Saving Algorithm to find the approximate
  * most frequent and top-k elements. The algorithm is described in "Efficient
  * Computation of Frequent and Top-k Elements in Data Streams". See here:
  * www.cs.ucsb.edu/research/tech_reports/reports/2005-23.pdf In the paper the
  * data structure is called StreamSummary but we chose to call it SpaceSaver
  * instead. Note that the adaptation to hadoop and parallelization were not
  * described in the article and have not been proven to be mathematically
  * correct or preserve the guarantees or benefits of the algorithm.
  */
sealed abstract class SpaceSaver[T] {
  ...
  /** Map of item to counter, where each counter consists of an observed count
    * and possible over-estimation (error)
    */
  ...
  /** Get the elements that show up more than thres times. Returns sorted in
    * descending order: (item, Approximate[Long], guaranteed)
    */
  ...
  /** Get the top-k elements. Returns sorted in descending order: (item,
  ...
  def topK(k: Int): Seq[(T, Approximate[Long], Boolean)] = {
    ...
    siK.map { case (item, (count, err)) =>
      (
        item,
        Approximate(count - err, count, count, 1.0),
        countKPlus1 < count - err
      )
    }
  }

  /** Check consistency with other SpaceSaver, useful for testing. Returns
    * boolean indicating if they are consistent
    */
...
object SSMany {
  ...
  private def bucketsFromCounters[T](
      counters: Map[T, (Long, Long)]
  ): SortedMap[Long, Set[T]] =

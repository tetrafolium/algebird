...
/** HLLSeries can produce a HyperLogLog counter for any window into the past,
  ...
  * This allows it to reconstruct an HLL as it would be had it started at zero
  * at any given point in the past, and seen the same updates this structure has
  * seen.
  *
  * @param bits
...
case class HLLSeries(bits: Int, rows: Vector[Map[Int, Long]]) {
  ...
  /** @param since
    ...
    *
    * @return
    *   New HLLSeries only including RhoWs for values seen at or after the given
    *   timestamp
    */
...
/** =Example Usage=
  ...
  *
  * val examples: Seq[Array[Byte], Long] val series = examples .map { case
  * (bytes, timestamp) => hllSeriesMonoid.create(bytes, timestamp) } .reduce {
  * hllSeriesMonoid.plus(_,_) }
  *
  * val estimate1 = series.since(timestamp1.toLong).toHLL.estimatedSize val
  * estimate2 = series.since(timestamp2.toLong).toHLL.estimatedSize
  */
class HyperLogLogSeriesMonoid(val bits: Int) extends Monoid[HLLSeries] {
  ...
  def create(example: Array[Byte], timestamp: Long): HLLSeries = {
    ...
    val vector =
      Vector.fill(rhow - 1) { Map[Int, Long]() } ++ Vector(Map(j -> timestamp))

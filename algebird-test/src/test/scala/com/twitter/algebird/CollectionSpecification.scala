...
class CollectionSpecification extends CheckProperties {
  ...
  implicit def arbAndVal: Arbitrary[AndVal] =
    Arbitrary {
      implicitly[Arbitrary[Boolean]].arbitrary.map { b => AndVal(b) }
    }
  ...
  property("Array Monoid laws") {
    monoidLawsEq[Array[Int]] { case (a, b) =>
      a.deep == b.deep
    }
  }
  ...
  property("MMap[Int,Int] Monoid laws") {
    isAssociativeDifferentTypes[ScMap[Int, Int], MMap[
      Int,
      Int
  ...
  property("MapAlgebra.toGraph is correct") {
    forAll { (l: Set[(Int, Int)]) =>
      (MapAlgebra
        .toGraph(l)
        .toIterable
        .flatMap { case (k, sv) => sv.map { v => (k, v) } }
        .toSet == l)
    }
  }
  ...
  property("MapAlgebra.join works") {
    forAll { (m1: Map[Int, Int], m2: Map[Int, Int]) =>
      ...
      val m1after =
        m3.mapValues { vw => vw._1 }.filter { _._2.isDefined }.mapValues {
          _.get
        }
      val m2after =
        m3.mapValues { vw => vw._2 }.filter { _._2.isDefined }.mapValues {
          _.get
        }
  ...
  property("MapAlgebra.mergeLookup works") {
    forAll { (items: Set[Int]) =>
      (mapEq.equiv(
        ...
        Map(
          (None: Option[Int]) -> Monoid.sum(
            items.map(x => square(x).getOrElse(0))
          )
        )
      ) && mapEq.equiv(
